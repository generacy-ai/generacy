...
## Cockpit gates — read-only status query + stable clarification generation (#1038, planning phase)
...
- Planned files (new): `packages/generacy/src/cli/commands/cockpit/mcp/tools/{cockpit_gate_status,cockpit_gate_list}.ts`, `packages/generacy/src/cli/commands/cockpit/mcp/gates/query-client.ts` (GET-shaped HTTP client with its own retry loop, sibling to existing POST-shaped `client.ts`), `packages/orchestrator/src/services/gate-status-query.ts` (correlation-id map + relay round-trip). Modified: `packages/cockpit/src/gates/generation.ts` (breaking `ClarificationGenerationInput` shape + sha256[:24] output), `packages/orchestrator/src/routes/cockpit-gates.ts` (adds `GET /cockpit/gates?issueRef=&mode=single|list&…` handler), `packages/generacy/src/cli/commands/cockpit/mcp/errors.ts` (`ErrorClass` gains `'query-unreachable'`), `packages/generacy/src/cli/commands/cockpit/mcp/server.ts` (registers two new tools alongside `cockpit_gate_open`/`cockpit_gate_ack`).
...
- **Integration coverage**: extends the #1024 harness with one gate-status scenario against the fake peer. End-to-end validation with real cloud lands when the generacy-cloud sibling responder merges.
