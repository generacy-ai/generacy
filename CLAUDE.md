...
## Cockpit gates — read-only status query + stable clarification generation (#1038)

- Part of the **Cockpit Remote Gates** epic (generacy-cloud#850) — dogfood follow-up from the `--gates=ui` sweep tracked in generacy-ai/agency#450. Design + wire: [docs/cockpit-remote-gates-plan.md](https://github.com/generacy-ai/tetrad-development/blob/develop/docs/cockpit-remote-gates-plan.md). Cluster-side dependency of the sweep-duplicate-drafting fix. Two coupled additions: (a) a body-free "is this gate open?" query so the sweep can skip drafting for already-pending gates, (b) a stable, durable-GitHub-state-derived `generation` for `clarification` and `implementation-review` so sweep-derived and live-derived `gateId`s coalesce.
- **Sweep primitive is `cockpit_gate_list` by `(issueRef, gateType)` prefix**, NOT `cockpit_gate_status` by full `gateId` (clarifications Q4=B / INV-5). Any currently-`open` gate for that pair suppresses drafting regardless of generation match. Kills the pre-existing `generation=1` cutover duplicate without either a cloud-side migration or a permanent legacy-ID compatibility shim.
- **Fail-loud on transport failure** (clarifications Q3=D / INV-2 / FR-011): both new tools NEVER return `absent` on transport failure. Bounded retry (~3 attempts spanning ~5s total: 500ms → 1500ms → 3000ms with ±10% jitter, per-attempt timeout 5000ms), then throw `class: 'query-unreachable'` — a new discriminated `ErrorClass` value. Sweep aborts the current cycle; auto-loop retries next tick when the relay is likely up.
- **Three-state response contract** (clarifications Q2=C / INV-3): cloud's 7-value gate-status enum collapses to `open | answered | absent`. Mapping: `open` = cloud `open`; `answered` = cloud `answered | delivered | applied`; `absent` = no matching gate OR terminal-negative (`superseded | failed | expired`) — sweep is free to re-draft a dead gate.
- **`cockpit_gate_list` scope** (clarifications Q5=A / INV-4): non-terminal only (`open | answered | delivered`, terminal statuses excluded server-side); **project-wide** (any cluster in the project — a serial-cluster takeover MUST see the predecessor cluster's still-open gates or it re-drafts everything). Cluster-scoped filtering would silently regress takeover-safety.
- **Clarification generation canonicalization** (clarifications Q1=A / FR-007): `ClarificationGenerationInput` shape changes from `{ batchId: string }` to `{ questions: Array<{ questionNumber: number; questionText: string }> }`. Canonical form = sorted ascending by `questionNumber`, re-emit each entry with FIXED key order (`questionNumber` first, `questionText` second) via explicit map before `JSON.stringify`, sha256, truncate to 24 hex chars. Drafted answers deliberately excluded — multiple sweeps of the same open batch produce the same generation. This IS the breaking change on `@generacy-ai/cockpit` — old callers passing `{batchId}` fail to compile.
- **Single-transport invariant** (spec Assumptions §96 / INV-6): query rides the SAME relay envelope path as `gate-open` and answers. No direct HTTPS-to-cloud fallback in v1 — single degraded-mode story. New envelope pair `gate_query_request` (cluster→cloud) / `gate_query_response` (cloud→cluster) added to `RelayMessageSchema` in `packages/cluster-relay/src/messages.ts` — NOT a reuse of `api_request`/`api_response` (those semantically describe cloud→cluster HTTP-dispatch, reversing them would invert the dispatcher contract in `packages/cluster-relay/src/proxy.ts:141`).
- Files added: `packages/generacy/src/cli/commands/cockpit/mcp/tools/{cockpit_gate_status,cockpit_gate_list}.ts`, `packages/generacy/src/cli/commands/cockpit/mcp/gates/query-client.ts` (GET-shaped HTTP client with its own retry loop, sibling to existing POST-shaped `client.ts`; exports `_setTestRetryBackoffs` for fast-forwarded unit tests), `packages/orchestrator/src/services/gate-status-query.ts` (correlation-id map + relay round-trip; exports `RelayClientForQuery` structural interface and `QueryUnreachableError` / `MalformedCloudResponseError`).
- Files modified: `packages/cockpit/src/gates/generation.ts` (breaking `ClarificationGenerationInput` shape + sha256[:24] output), `packages/orchestrator/src/routes/cockpit-gates.ts` (adds `GET /cockpit/gates?issueRef=&mode=single|list&…` handler; `SetupCockpitGatesRouteOptions` gains optional `getQueryService`), `packages/generacy/src/cli/commands/cockpit/mcp/errors.ts` (`ErrorClass` gains `'query-unreachable'`), `packages/generacy/src/cli/commands/cockpit/mcp/server.ts` (registers two new tools alongside `cockpit_gate_open`/`cockpit_gate_ack`), `packages/cluster-relay/src/messages.ts` (adds `gate_query_request` / `gate_query_response` interfaces and Zod schemas to `RelayMessage` + `RelayMessageSchema`; `parseRelayMessage` cross-field-guards `status='ok'` requires `payload`), `packages/cluster-relay/package.json` (new `@generacy-ai/cockpit` workspace dep for `GateType` / `GateTypeSchema`), `packages/orchestrator/src/server.ts` (instantiates `GateStatusQueryService`, threads it through `initializeRelayBridge` + `activateInBackground` so both wizard and existing-key paths wire `client.on('message', ...)` to the service).
- **No CLI twin** for the two new MCP tools (spec R8, matches `cockpit_gate_open`/`cockpit_gate_ack` precedent at `mcp/server.ts:194-204`) — they're only meaningful inside an active `/cockpit:auto` session or an operator's Claude Code MCP context. Mocked-orchestrator unit tests cover the same code paths a CLI twin would.
- **Parity fixture** for INV-1 / SC-002 in `packages/cockpit/src/gates/__tests__/gates-generation.test.ts`: given a fixed `{questions:[…]}` and a fixed `headSha`, `deriveGateId(deriveGateKey(...))` returns a byte-identical string as a reference hash committed in the test. Any drift in `deriveClarificationGeneration` canonicalization or key order breaks the test — proves sweep + live paths hash identical bytes.
- **Integration coverage**: #1024 harness extended with a `Q1` scenario. `FakePeer` gains `setGateQueryResponder(handler)` — registers a callback that receives the `gate_query_request` and returns the response payload; peer wraps with the correct `type` and correlation id. Scenario helper (`scenario-helpers.ts`) instantiates `GateStatusQueryService`, threads it through `setupCockpitGatesRoute` via `getQueryService`, and registers the client's `message` handler so `gate_query_response` frames route back through the pending promise. End-to-end validation with real cloud lands when the generacy-cloud sibling responder merges.
- **Wiring**: orchestrator's `initializeRelayBridge` gains an optional `gateStatusQuery` parameter (last arg); when passed, it registers `client.on('message', service.onRelayMessage.bind(service))` after the client is constructed. `activateInBackground` mirrors the parameter and forwards it. Both wizard-mode and existing-key-mode paths in `createServer()` construct one `GateStatusQueryService` instance shared across the two branches.
- Changeset: `.changeset/1038-cockpit-gate-status-query.md` — `@generacy-ai/generacy` **minor** (new MCP tools + orchestrator route), `@generacy-ai/cockpit` **minor** (breaking `deriveClarificationGeneration` input-shape change; `## Breaking changes` section in the changeset body with the before/after migration snippet for the agency-side paired PR), `@generacy-ai/cluster-relay` **patch** (additive envelope pair, no removed types). Single changeset lists all three packages per CLAUDE.md gate.
- **Cross-repo dependencies** (informational): (a) `generacy-ai/generacy-cloud` sibling ships the cloud-side `gate_query_request` responder — a Firestore query by `(issueRef, gateType?)` gated by project membership; wire contract lives in `specs/1038-part-cockpit-remote-gates/contracts/gate-query-relay-envelope.md`. (b) `generacy-ai/agency` sibling replaces `generation=1` hard-code in `packages/claude-plugin-cockpit/commands/auto.md:198` with `deriveClarificationGeneration` + `deriveImplementationReviewGeneration` calls, and inserts the `cockpit_gate_list` skip-drafting check per INV-5.
