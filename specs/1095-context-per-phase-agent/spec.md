...
| SC-006 | `generacy validate` rejects unknown keys under `agents` / `workflows.*` / `phases.*` / entries (strict / no-passthrough). | Rejected | CLI test with a typo fixture (e.g. `defualt:` instead of `default:`). |
...
7. **`generacy validate` is strict on unknown keys.** The CLI authoring schema in `packages/generacy/src/config/schema.ts` uses (or should use) `.strict()` / no-passthrough Zod semantics so that typos surface as errors instead of silent strips. **/plan** should confirm the current mode and preserve or enforce strictness on the `agents` sub-tree.
