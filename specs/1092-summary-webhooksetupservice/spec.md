...
| FR-002 | `_ensureWebhookForRepo`'s `skip-active` branch MUST detect `missingEvents = LOCKED_EVENTS.filter(e => !hook.events.includes(e))` and, when non-empty, PATCH the hook to `events = [...new Set([...hook.events, ...LOCKED_EVENTS])]` via the existing `_updateRepoWebhook(owner, repo, id, { events })` helper (events-only PATCH — same helper the adjacent reactivate branch uses; do NOT rewrite `config.url` / `content_type`). | P1 | Resolved by clarification Q2 → A. No new gh-cli surface required. |
| FR-003 | When FR-002 issues a PATCH, the outcome MUST be counted as `WebhookSetupSummary.reactivated++` and reported as `action: 'reactivated'` on `WebhookSetupResult`, matching the URL-heal precedent. The event-heal case is discriminated in operator triage via the log-line MESSAGE string (see FR-004), not via a distinct action value. | P1 | Resolved by clarification Q1 → A. Preserves the public `WebhookSetupResult.action` union and all existing `.reactivated` counter assertions; extend the doc comment on the counter to name the event-heal case. |
| FR-004 | The `skip-active` mismatch branch MUST replace the existing `logger.warn(..., 'Existing webhook has event mismatch - events not updated')` with a single `logger.info(..., 'Existing webhook was missing events — patched')` line carrying `{owner, repo, webhookId, missingEvents, newEvents}`. The stale `expectedEvents: ['issues']` field is dropped. | P1 | Resolved by clarification Q3 → C. A successful self-heal is not warn-level; mirrors the URL-heal's single-info precedent (oldUrl/newUrl). Promoted from P2 → P1 since it is now the sole log signal for the heal branch. |
...
| SC-005 | Synthetic `pull_request_review.submitted` and `issue_comment.created` payloads dispatched through `smee-receiver.ts`'s per-event table land on the correct monitor mock. | 100% of the three new event types covered by a unit test. | Unit tests extending the pattern in `smee-receiver-987.test.ts` (tests 6–7). Live-cluster end-to-end verification remains a post-deploy operator smoke check, not a CI gate. (Resolved by clarification Q4 → C.) |
...
4. Resolved by clarification Q1 → A: the event-heal case reuses `action: 'reactivated'` and increments `WebhookSetupSummary.reactivated`. Discrimination for operator triage is provided by the log-line message string (FR-004), not by a distinct action value. No public API widening.
