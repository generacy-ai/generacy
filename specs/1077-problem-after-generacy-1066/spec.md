...
At each outbound send site, generate a per-frame id and attach it inside the
wire body's `data`:
...
The id must be non-empty, unique per send (retries of the same
natural gate reuse the natural `gateId` — the *frame* around a retry is a
distinct send and gets a distinct `frameId`), and stable within a single call
so that the id reported to the caller matches the id on the wire.

### 2. Consume

At `packages/cluster-relay/src/relay.ts:334`, keep a `Map<frameId, pending>`
and settle the pending entry when a matching `cluster.cockpit.reply` arrives,
replacing the current log-and-return branch. An entry whose `frameId` matches
nothing (caller gone, TTL expired, post-reconnect drain of a frame whose
sender is no longer waiting) must be dropped **quietly** — no error, no
leaked map entry, no log at `error` level.
...
## Open design questions for the clarify gate

The seam between "mint at the MCP tool" and "consume at the relay client"
crosses a process boundary: `POST /cockpit/gates` is a synchronous loopback
call returning `202 {accepted, retained}`, while `cluster.cockpit.reply`
arrives *later*, over the WebSocket, in a different place. The MCP tool that
sent the frame has already returned to its caller by the time the reply
arrives. This produces a set of coupled decisions that /speckit:clarify must
land:

- **Q1 — Who mints?**
  - **(a) Caller-mints** — the MCP tool generates the id. Matches #1066's
    "caller-supplied" framing. Requires a way for the caller to observe the
    reply (options in Q2). Note: agency#469's `/cockpit:auto` flow, MCP
    doorbell, and ad-hoc callers all mint separately — the tool boundary is
    the natural home.
  - **(b) Route-mints** — the orchestrator route
    (`packages/orchestrator/src/routes/cockpit-gates.ts`) generates the id
    when it emits, returns it on the 202, and the relay client owns the
    pending map. Simpler wiring; makes #1066's caller-supplied field
    vestigial unless kept as an override.
- **Q2 — How does the sender learn the outcome?** (Coupled to Q1.)
  - Field on the 202 response body plus a follow-up read?
  - The existing `/events` SSE stream?
  - A small `GET /cockpit/gates/frames/:frameId` endpoint?
  - The relay client emits an event the MCP tool subscribes to before send?
- **Q3 — Where does the pending map live and how long?**
  - Relay client owns it (matches the receive site).
  - TTL bound so a caller that has walked away doesn't leak. What TTL?
  - Post-reconnect drain of retained frames: their `frameId` is preserved
    verbatim (#1066 FR-008), but their original sender may have TTL'd. The
    unknown-`frameId`-arrival path must be quiet-drop, not error.
- **Q4 — Wire location of `frameId` on the frame body.** #1066 established
  that `frameId` sits *inside* `data` on the outbound relay frame, co-located
  with `gateId`, `gateType`, etc. Q4 is a confirmation, not a new decision:
  minting must place the field there, not on the `EventMessage` envelope.

Whichever mint scheme wins, the retain-and-replay path already preserves
...
### US2 — Cluster relay client correlates `cluster.cockpit.reply` back to the mint

**As** the cluster-side relay client (`packages/cluster-relay/src/relay.ts`),
**I want** every incoming `cluster.cockpit.reply` to be looked up in a
`frameId → pending` map and settled on match,
**So that** the sender (or whoever is waiting on the outcome) learns whether
the frame was accepted or dropped, distinct from other retries of the same
`gateId`.

**Acceptance criteria**:
- A reply whose `frameId` matches an entry in the pending map settles that
  entry with the reply's fields (accepted / reason / priorStatus, etc.).
- A reply whose `frameId` matches nothing is dropped quietly: no throw, no
  error-level log, no map mutation.
- After settle or TTL, the map entry is removed — no leak across many
  send-then-reply cycles.
- The relay client remains usable when no sender is registered for a given
  `frameId` (e.g., short-lived MCP-tool caller that has already returned).
...
|----|-------------|----------|-------|
| FR-001 | Every outbound `gate-open` frame carries a non-empty `frameId` string inside `data`. | P1 | `packages/generacy/src/cli/commands/cockpit/mcp/tools/cockpit_gate_open.ts`. Wire location per #1066 FR-007. |
| FR-002 | Every outbound `gate-outcome` frame carries a non-empty `frameId` string inside `data`. | P1 | `packages/generacy/src/cli/commands/cockpit/mcp/tools/cockpit_gate_ack.ts`. |
...
| FR-004 | The relay client maintains a `frameId → pending` map and settles the matching entry on `cluster.cockpit.reply`. | P1 | `packages/cluster-relay/src/relay.ts:334`. Replaces the current log-and-return branch. |
| FR-005 | A `cluster.cockpit.reply` whose `frameId` matches nothing is dropped quietly (no throw, no error log, no map mutation, no leak). | P1 | Covers: caller gone before reply, TTL expired, post-reconnect drain. |
| FR-006 | The pending-map entry is removed on settle. | P1 | No leak across many cycles. |
| FR-007 | The pending-map entry is removed on TTL expiry. | P1 | Bound TTL — value to be set by /speckit:clarify. |
...
- The `POST /cockpit/gates` route's parsing / forwarding contract — settled
  by #1066 FR-005 and not re-opened here.
- Whether the caller *acts* on the correlated outcome (e.g., a doorbell that
  waits for `accepted` before continuing). This spec guarantees the settle;
  what a specific caller does with it is that caller's ticket.
