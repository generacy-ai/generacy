...
- [X] T009 Run `pnpm --filter @generacy-ai/orchestrator typecheck` and `pnpm --filter @generacy-ai/orchestrator test` from repo root. All three new test files must pass; no existing tests may regress. Depends on T006 + T007 + T008.

- [X] T010 [US1] SC-002 static verification: `grep -RIn "AllowGateComplete.CockpitAdvance" packages/ --include='*.ts' --exclude-dir='__tests__'` MUST return **zero** hits (the token exists as a type export only; no production caller uses it today). Depends on T003.

- [X] T011 [US1] SC-003 deliberate-regression check: temporarily insert `await github.addLabels(owner, repo, issueNumber, ['completed:implementation-review']);` into `PrFeedbackHandler.handle()`'s happy-path branch, run the FR-005 test from T008 — it MUST go red with a message naming the offending write. Revert the insertion before committing. Depends on T008.
