...
| FR-002 | Before any `addLabels(...)` call from the phase loop's label boundary, the target repo is confirmed to have the labels — either by proactive sync (`LabelSyncService.syncNewRepo`) or reactive create-if-missing at the `LabelManager` boundary. | P1 | Prevents recurrence for every future protocol addition without requiring a #889-style hotfix. |
| FR-003 | When `LabelManager` exhausts its 3-attempt retry inside `onGateHit`/`onPhaseStart`/`onPhaseComplete`/`onError`, the phase loop does NOT re-throw out of `ClaudeCliWorker.processItem`. Instead, the item enters the terminal failed state (agent:error applied via the best-effort path, item marked completed-with-failure from the queue's perspective so it is not re-released). | P1 | Direct fix for the observed crash-loop. |
| FR-004 | The failure surface for FR-003 includes a #865-style alert comment on the issue that names the failed label operation (`onGateHit(implement, waiting-for:merge-conflicts)`) and includes the `gh` stderr as evidence. | P2 | Operator-facing diagnosability; parallel to the #865/#847 evidence contracts. |
| FR-005 | A regression test asserts that on a repo lacking `waiting-for:merge-conflicts`, a base-merge conflict pauses successfully (label created before add, or synced up-front). | P1 | Locks in FR-001+FR-002. |
| FR-006 | A regression test asserts that a mocked hard failure of `addLabels` after 3 retries fails the item gracefully (no unhandled throw, no queue release, item ends in `agent:error`, worker continues to next item). | P1 | Locks in FR-003. |
| FR-007 | A drift/audit test enumerates every label symbol the orchestrator applies (`phase:*`, `completed:*`, `waiting-for:*`, `failed:*`, `agent:*`) and asserts each is present in `WORKFLOW_LABELS`. | P1 | Locks in US3; prevents the class of bug from recurring. |
