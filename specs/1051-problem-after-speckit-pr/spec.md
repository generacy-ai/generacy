...
This spec bundles three independent fixes that eliminate each contributor plus a dispatch-time gate that stops the doomed job before it runs.

**Retraction** (PR #1052 review Finding 5): the third contributor in the original writeup — "reused checkout carrying another issue's working tree" — was inferred from a misreading of `d8e392ca`. `gh api repos/generacy-ai/generacy-cloud/commits/d8e392ca --jq '.parents[].sha'` returns TWO parents (`c3cbe0e4` and `5542e900`); it is a routine base-merge of `develop`, not a single-parent commit with a contaminated working tree. The `added` file statuses that produced the misreading are an API artifact (GitHub diffs merge commits against parent 1 only). Corroborated by generacy-cloud#886 which resurrected `884-problem-refreshaccesstoken` with no foreign files at all. FR-004 and SC-003 have been cut; the corresponding regression test at `packages/orchestrator/src/__tests__/repo-checkout-cross-issue.test.ts` has been deleted.
...
`git compare develop...d8e392ca` → `diverged, ahead_by=9, behind_by=1`. Nine commits that can never land on develop because the branch was already merged and closed.

Corrected reading (PR #1052 review Finding 5): `d8e392ca` is a **merge commit** with two parents (`c3cbe0e4` = pre-merge tip of `879-problem-any-4xx-from`, `5542e900` = squash-merge of PR #881 = #880's landed work). The "every file belongs to #880" observation is an artifact of `gh api ... commits/d8e392ca` diffing against parent 1 only — files coming in via parent 2 render as `added`. This is the `#864` pre-phase hook doing a routine base merge of `develop` into the branch. No cross-issue file contamination occurred. Corroborated by generacy-cloud#886 which resurrected `884-problem-refreshaccesstoken` with no foreign files. The commit `d8e392ca` was authored 63 s after PR #882's merge (12:53:26 vs 12:52:23), and PR #883 opened 6 s later — measured merge-to-duplicate-PR windows across two field instances are 69 s (#883) and 71 s (#886).
...
- [ ] Decision matrix on the PR-lookup result:
  - `null` (no PR in any state) → **allow**, regardless of remote branch presence (first-push case — brand-new issue's local branch has not yet been pushed to origin; PR #1052 review Finding 1)
  - `OPEN` + branch present → **allow**
  - `OPEN` + branch missing → **refuse** with `reason: 'branch-missing'`
...
| FR-004 | ~~On checkout reuse, the working tree MUST be guaranteed to contain only files scoped to the target issue at the moment the phase step begins.~~ **RETRACTED** (PR #1052 review Finding 5) — the contamination premise was wrong; `d8e392ca` is a base-merge commit, not a single-parent commit with a contaminated tree. No fix required. | — | US3. |
| FR-005 | A phase job whose target issue is closed MUST be dropped at dispatch (enqueue time). This applies to **both** `type === 'process'` and `type === 'resume'` events in `LabelMonitorService.processLabelEvent()` (Q1 clarification — observed repro was a resume event). The drop emits one `info` log line with structured field `dropped: 'issue-closed'` plus `{ issueNumber, eventType, phase }`. Drop is silent from a mutation standpoint: no checkout, no push, no PR, no `agent:error`. | P1 | US4. Defense-in-depth. Scope note: four other enqueue paths (`base-advance-monitor-service` resume, `worker-dispatcher` lease-expiry re-enqueue, `worker-dispatcher` post-complete rearm, `pr-feedback-monitor-service`) do NOT flow through `processLabelEvent` and are out of scope for this spec — see PR #1052 review Finding 9. |
...
| SC-002 | A push targeting a branch whose PR is merged/closed is refused; the refusal is logged at `warn` or above with the FR-003a event shape. | 100% of attempted pushes to a merged-PR branch are refused; every refusal emits exactly one `warn`-or-above line matching `event: 'push-refused'` with a valid `reason` enum value. | Unit test on the pre-push guard covering `{PR merged + branch present, PR closed + branch present, PR merged + branch missing, PR open + branch missing (refuse), PR open + branch present (allow), no PR + branch present (allow), no PR + branch missing (allow — first-push case, #1052 Finding 1)}` matrix. |
| SC-003 | ~~Files belonging to issue B cannot land in a commit on issue A's branch.~~ **RETRACTED** (PR #1052 review Finding 5) — mechanism never existed. Test file `repo-checkout-cross-issue.test.ts` deleted. | — | — |
| SC-004 | A phase job for a closed issue is **dropped at enqueue time** (inside `LabelMonitorService.processLabelEvent`) with an `info` log line naming the reason. Applies to **both** process and resume events emitted through the label-monitor path. | 100% of enqueue attempts through `processLabelEvent` whose target issue is closed exit with 0 mutations (no checkout, no push, no PR, no label mutation). Coverage is enqueue-time, not worker-claim time (PR #1052 review Finding 8) — a worker-claim-time gate would additionally protect against issues that close between enqueue and claim; that is a follow-up scoped separately. | Unit test on `processLabelEvent`: enqueue a phase job for a pre-closed issue for each of (a) `type: 'process'` and (b) `type: 'resume'`, assert `queueManager.enqueue` uncalled and the log line is present with fields `{ dropped: 'issue-closed', issueNumber, eventType, phase }`. |
