...
- [X] T001 [Setup] Add the required changeset file `.changeset/1107-implement-product-diff-guard.md`:
...
- [X] T002 [P] [Foundation] Add two local-git methods to the `GitHubClient` interface in
...
- [X] T003 [Foundation] Implement both methods in
...
- [X] T004 [P] [Foundation] Add raw-key string get/set/clear to
...
- [X] T005 [P] [US1] In `packages/orchestrator/src/worker/product-diff.ts`, add the exact-filename exclusion:
...
- [X] T006 [US2] In `packages/orchestrator/src/worker/product-diff.ts`, add
...
- [X] T007 [P] [Wiring] In `packages/orchestrator/src/worker/types.ts`, add optional
  `phaseTracker?: PhaseTracker` to `PhaseLoopDeps`.

- [X] T008 [Wiring] In `packages/orchestrator/src/worker/claude-cli-worker.ts`, pass the existing
  `this.phaseTracker` into `PhaseLoopDeps` at the phase-loop construction site. (Depends on T007)

- [X] T009 [Wiring] In `packages/orchestrator/src/server.ts`, thread the already-constructed
...
- [X] T010 [US2] In `packages/orchestrator/src/worker/phase-loop.ts`, capture / persist / reuse the phase-start ref.
...
- [X] T011 [US1] In `packages/orchestrator/src/worker/phase-loop.ts` step 5b guard (`:709-784`), switch the guard to the
...
- [X] T012 [US1] In `packages/orchestrator/src/worker/phase-loop.ts`, extend the failure diagnostics (FR-004) at the
...
- [X] T013 [P] [US1] Unit tests in `packages/orchestrator/src/worker/__tests__/product-diff.test.ts` for the exclusion
...
- [X] T014 [P] [Foundation] Unit tests in
...
- [X] T015 [US2] Integration tests in
...
- [X] T016 [P] [US3] Confirm the healthy path and detection-failure path stay green:
...
- [X] T017 [Verify] Run the full gate locally and confirm all success criteria:
