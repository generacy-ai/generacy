...
**Answer**: **Option A: Shared SSE via SSESubscriptionManager.** Follow the `AgentLogChannel` pattern — subscribe to a new `'jobs'` channel, filter by `jobId`. The server-side `SSESubscriptionManager` already supports `broadcastFiltered()` for per-job targeting, and the client-side manager handles reconnection and `Last-Event-ID` replay. One shared connection is simpler to manage. High-volume log data can be mitigated by server-side filtering so only events for subscribed jobIds are sent.
...
**Answer**: **Option B: Pre-cleaned plain text.** The server strips ANSI codes and non-printable characters before sending. OutputChannel doesn't support ANSI rendering, so the server is the natural place to do cleanup. The extension displays content as-is without needing an ANSI parsing dependency.
...
**Answer**: **Option C: Fetch last N lines, no pagination.** Fetch the most recent 10,000 lines in a single request. If truncated, show a `--- Showing last 10,000 of {total} lines ---` header. No load-more in v1. This matches the simplicity of `AgentLogChannel` (which fetches 200 historical lines) scaled up appropriately. Pagination adds complexity that isn't warranted until real usage patterns emerge.
...
**Answer**: **Option C: Dispose on session end.** Matches the existing `AgentLogChannel` pattern — maintain a static `activeChannels` map and provide `disposeAll()` called on extension deactivation. No mid-session auto-disposal avoids surprising users who are reading completed job output. OutputChannel resource usage is minimal for idle channels.
...
**Answer**: **Option B: Server-side cursor.** The historical REST response should include a `cursor` or `lastEventId` that can be passed to the SSE subscription as a query parameter (e.g., `?since={cursor}` or as initial `Last-Event-ID`). This gives a clean handoff with zero gap and zero duplication, without client-side dedup complexity. The server-side `SSESubscriptionManager` already maintains an event buffer that supports replay from a given event ID. This is being co-designed with #178.
...
**Answer**: **Option A: Always visible, show empty state.** Show the button for all job states. For pending/queued jobs, open the channel with `"Waiting for job to start..."` and auto-populate when logs arrive. This is the most intuitive UX — similar to opening a terminal before a process starts. Users can proactively open the log viewer and not miss the beginning of output.
...
**Answer**: **Option B: Explicit boundary events.** Since we control the SSE protocol design in #178, the server should emit distinct `job:step-start` / `job:step-end` events alongside `job:log` events. This cleanly separates content from structure. The server knows authoritatively when steps transition — relying on client-side `stepName` diffing is fragile, and cross-referencing progress events introduces coupling between channels.
...
**Answer**: **Option A: First 8 chars of UUID.** `"Job: my-workflow (a1b2c3d4)"` — concise, reliably unique, matches git short-hash conventions that developers are familiar with. No state tracking needed (unlike sequential numbers), and no ambiguity (unlike timestamps that could collide within a minute).
...
**Answer**: **Option C: Graceful degradation with retry.** Open the channel, display the error inline, and retry up to 3 times with backoff. This is consistent with how `SSESubscriptionManager` already handles reconnection — the user sees what's happening without needing to manually retry. A toast-only approach loses context, and no-retry is frustrating when the endpoint might become available moments later (e.g., worker startup).
...
**Answer**: **Option A: No limit in v1.** VS Code's OutputChannel is designed for large text and handles it efficiently. Adding rotation or caps adds complexity for a problem that may not materialize in practice. Document the limitation and revisit if actual usage shows memory pressure. Premature optimization here would add code paths that are hard to test and may never be needed.
...
**Answer**: **Option C: Event-driven from log stream end.** The SSE stream should send a final `job:log:end` event with terminal status when the job completes. `JobLogChannel` handles this event to display final status (e.g., `"--- Job completed ---"`) and stops listening. This is self-contained — no coupling to `JobDetailPanel`, no shared state service. Each component manages its own lifecycle independently, matching the existing architecture.
