...
- [X] T001 [US1] FR-007 grep-audit: search `packages/orchestrator/src` and `packages/workflow-engine/src` for any self-recognition consumer on the pr-feedback surface that does not go through `getPRReviewThreads()` (the thread-shaped GraphQL client). Search terms: `clusterIdentity`, `resolveActingIdentity`, `CLUSTER_ACTING_LOGIN`, `normalizeLogin(.*actingLogin|.*clusterIdentity)`. Expected result: only the sites enumerated in plan.md. If a straggler surfaces, migrate it to `getPRReviewThreads()` in the same PR (FR-007 Q2→B remedy) rather than halting.

## Phase 2: GraphQL Query & Type Extensions

- [X] T002 [US1] Add optional `viewerDidAuthor?: boolean` field to `Comment` interface in `packages/workflow-engine/src/types/github.ts` (~lines 72–94). Include the JSDoc block from `data-model.md` describing "populated by `getPRReviewThreads()` only" semantics.

- [X] T003 [US1] Extend `getPRReviewThreads()` in `packages/workflow-engine/src/actions/github/client/gh-cli.ts` (~lines 479–576):
...
- [X] T004 [US1] Add gh-cli tests covering `viewerDidAuthor` propagation from the mocked `gh api graphql` response into the parsed `Comment[]` — four rows from `contracts/review-thread-query.contract.md`: `true` → `true`, `false` → `false`, `null` → absent, missing key → absent.

## Phase 3: Trust Predicate Replacement

- [X] T005 [US1] Rename `TrustReason` union entry `'cluster-identity'` → `'self-authored'` in `packages/workflow-engine/src/security/comment-trust.ts`. Hard rename per Q1→D — no dual-emit. TS compiler flags every consumer; fix them in the same commit.

- [X] T006 [US1] Remove the `clusterIdentity?: string` field from `CommentTrustContext` in `packages/workflow-engine/src/security/comment-trust.ts`. Callers stop threading it; the self-authored signal now lives on the comment itself.

- [X] T007 [US1] Replace decision 1.5 in `isTrustedCommentAuthor()` in `packages/workflow-engine/src/security/comment-trust.ts` per `contracts/trust-predicate.contract.md`:
...
- [X] T008 [US1] Rewrite `packages/workflow-engine/src/security/__tests__/comment-trust.test.ts`:
...
- [X] T009 [US1] Delete `packages/orchestrator/src/services/acting-identity.ts` and `packages/orchestrator/src/services/__tests__/acting-identity.test.ts`. No replacement — the mechanism they serve is dissolved.

- [X] T010 [US1] Update `packages/orchestrator/src/server.ts` (~lines 45, 172, 347, 416):
...
- [X] T011 [US1] Update `packages/orchestrator/src/services/pr-feedback-monitor-service.ts` (~lines 95, 103, 216–221, 298–314) per `contracts/skip-warn-shape.contract.md` Site 1:
...
- [X] T012 [US1] Update `packages/orchestrator/src/services/__tests__/pr-feedback-monitor-service.test.ts` (~lines 1677–1715) to match the new constructor signature and skip-warn evidence shape from T011.

- [X] T013 [US1] Update `packages/orchestrator/src/worker/pr-feedback-handler.ts` (~lines 79, 123–131, 169, 192–227, 272–288) per `contracts/skip-warn-shape.contract.md` Sites 2 & 3:
...
- [X] T014 [US1] Update `packages/orchestrator/src/worker/__tests__/pr-feedback-handler.test.ts` (~line 1023) to match the new constructor signature and evidence shape from T013.

- [X] T015 [US1] Update `packages/orchestrator/src/__tests__/pr-feedback-integration.test.ts` fixtures and assertions: replace `'cluster-identity'` reason strings with `'self-authored'`; replace `clusterIdentity` context threading with per-comment `viewerDidAuthor` on fixture comments; assert the SC-001 scenario (only unresolved comment has `viewerDidAuthor: true`, `authorAssociation: 'NONE'` → trusted, handler proceeds).

## Phase 5: Scaffolder Cleanup (generacy CLI)

- [X] T016 [P] [US1] Delete `actingLogin?: string` from `ScaffoldEnvInput`, the `actingLoginLines` computation, and the `CLUSTER_ACTING_LOGIN=…` interpolation in generated `.env` in `packages/generacy/src/cli/commands/cluster/scaffolder.ts` (~lines 54–60, 347–350, 358).

- [X] T017 [P] [US1] Delete `actingLogin: config.actingLogin` forwarding in `packages/generacy/src/cli/commands/launch/scaffolder.ts` (~line 114).

- [X] T018 [P] [US1] Delete `actingLogin: config.actingLogin` forwarding in `packages/generacy/src/cli/commands/deploy/scaffolder.ts` (~line 73).

- [X] T019 [P] [US1] Delete `actingLogin: z.string().min(1).optional()` from `LaunchConfigSchema` in `packages/generacy/src/cli/commands/launch/types.ts` (~line 75).

- [X] T020 [US1] Update `packages/generacy/src/cli/commands/cluster/__tests__/scaffolder.test.ts` (~lines 735–800): delete the 4 `CLUSTER_ACTING_LOGIN` tests (absent / present / whitespace / raw-value-written); add a negative test asserting the generated `.env` never contains the string `CLUSTER_ACTING_LOGIN`.

## Phase 6: Polish

- [X] T021 [US1] Run SC-003 grep-audit: `grep -r CLUSTER_ACTING_LOGIN packages/*/src` must return zero matches. If a straggler surfaces, remove it before opening the PR.

- [X] T022 [US1] Add one-line notes to the changeset:
