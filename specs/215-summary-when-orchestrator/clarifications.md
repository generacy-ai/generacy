...
**Answer**: **Option A (Resume at the NEXT phase)**. The clarify phase already ran to completion — the gate fired *after* it succeeded. Resuming at `clarify` would re-execute it and hit the same `always` gate again, creating an infinite loop. Returning `'plan'` (the next phase) follows the exact pattern already established by `reviewToPhase`: `'spec-review' → 'clarify'` means "after spec-review, start at clarify." Adding `'clarification' → 'plan'` is consistent and requires a one-line change.
...
**Answer**: **Option B (Unified map)**. A single `GATE_MAPPING` with both fields (`{ 'clarification': { phase: 'clarify', resumeFrom: 'plan' } }`) eliminates the ambiguity between "which phase is gated" and "where to resume after." Two separate maps with similar structures and subtly different semantics are a bug waiting to happen. The unified map is self-documenting and prevents the two concerns from drifting out of sync.
...
**Answer**: **Option D (Default to `speckit-feature`)**. During the transition period, existing issues won't have the `workflow:` label. Option C (skip with warning) is too strict — it blocks all pre-existing issues from resuming. Option B won't work because `process:*` labels are removed after processing. Defaulting to `speckit-feature` with a warning log is pragmatic — it's the overwhelmingly common case, and the warning makes it visible when the fallback is used. This is explicitly temporary behavior for backward compatibility.
...
**Answer**: **Option A (In label-monitor-service on `process:` events)**. The workflow name is already known when the label monitor processes a `process:speckit-feature` event — it's the `parsedName`. Applying `workflow:speckit-feature` right there, before enqueueing, guarantees the label exists before any worker runs. No changes needed in the worker or label manager. Single, focused change in one file.
...
**Answer**: **Option A (Already guaranteed by dedup tracker)**. The dedup tracker prevents duplicate processing for the same issue+phase combination. If `completed:clarification` fires twice rapidly, the second event is marked as duplicate and skipped. For `process:` events, the dedup key is cleared first, then re-marked after enqueueing. The practical risk is negligible.
...
**Answer**: **Option A (Yes, update it)**. Since we're adding the `workflow:` label and the PR feedback monitor already has its own `resolveWorkflowName()` doing the same label-scanning logic, reading the `workflow:*` label there is a trivial change that prevents a predictable follow-up bug. Keep it in scope.
...
**Answer**: **Option C (Implementer's choice)**. This is cosmetic and doesn't affect functionality.
...
**Answer**: **Option A (Only handle `always` for now)**. The phase loop only pauses for `always` gates. Other conditions aren't implemented and aren't triggered. Don't design for hypotheticals.
...
**Answer**: **Option B (Implied by the changes)**. Tests that assert buggy behavior will fail during development. The implementer will fix them naturally. Explicitly listing them adds spec maintenance burden for no real benefit.
...
**Answer**: **Option A (After phase resolution, before execution)**. Removing `waiting-for:*` and `agent:paused` right after `resolveStartPhase` succeeds (but before the loop starts) is a clear, one-time cleanup at the start of a resume. It's explicit about what's happening and removes stale labels before any phases run.
