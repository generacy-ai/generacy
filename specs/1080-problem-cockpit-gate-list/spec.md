...
| FR-002 | Rewrite the docblock on `CockpitGateListInputSchema.runId` at `packages/generacy/src/cli/commands/cockpit/mcp/gates/query-schemas.ts:65-75` to match FR-001's rationale — or delete field-level prose and cross-link to the handler-site comment as source of truth. | P1 | The two prose sites must not disagree post-fix. |
| FR-003 | Locate every test that pins the `runId` drop by asserting the cloud's 400 rationale, and rewrite each to assert `client.listGates` is invoked without `runId`. | P1 | If no such test exists (some sites in the issue Ask #3 are hypothetical — "*Any* test that pins the drop by asserting the 400 rationale rather than the drop itself"), add a positive spy-based test that pins the drop for regression coverage. |
...
| SC-002 | The rewritten comment at `cockpit_gate_list.ts:50-60` names agency#471 (or the "startup-sweep adoption" concept) explicitly. | 1+ occurrence | grep for `agency#471` or `startup-sweep` in the file returns ≥1. |
| SC-003 | A test in the suite asserts `client.listGates` is called without `runId` on its query argument (spy/mock at the seam). | ≥1 test | New or rewritten test file in `packages/generacy/src/cli/commands/cockpit/mcp/tools/__tests__/` (or the current test location — TBD at /plan). Runs green against unchanged handler code. |
...
1. **The three site references are exhaustive for this repo.** The issue lists three sites (`cockpit_gate_list.ts:50-60`, `query-schemas.ts:65-75`, "Any test that pins the drop by asserting the 400 rationale"). No other file in `packages/generacy/src/` documents or tests the drop with the vanished-refine framing. **/plan** should verify with a repo-wide grep for `runId requires generation`, `RFC-7807`, and `would 400`.
2. **The drop happens at exactly one location.** `cockpit_gate_list.ts:50-60` is the sole handler-side site where `runId` is stripped before forwarding to the cloud. **/plan** should verify with a search for `runId` in `packages/generacy/src/cli/commands/cockpit/mcp/tools/` and adjacent client-wrapper code.
3. **The test seam is `client.listGates`.** The issue names `client.listGates` as the mock/spy point for the rewritten test. **/plan** should confirm the actual interface name and import path.
4. **generacy-cloud#894 is not merged/deployed yet** ("Blocked on generacy-cloud#894 merging and deploying to staging"). The issue is filed proactively so the stale prose is tracked from the moment it becomes stale. This spec can land ahead of #894 deploying — the drop is correct in both worlds, only the *justification* changes. There is no runtime dependency on #894 for this PR to be correct.
5. **The agency-side fix (`auto.md:86`) does not block this PR.** It's a separate-repo change and per the issue it can be a "sibling" issue tracked independently.
6. **No new label vocabulary in `workflow-engine` is added.** Per CLAUDE.md's changeset rules, this makes the changeset a `patch` (documentation and test changes; no public API surface change). If it turns out `FR-003` requires new public test utilities, the bump may need to be `minor` — verified at implement time.
