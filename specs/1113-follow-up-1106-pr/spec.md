...
- [ ] A test populates the `prev` `SnapshotMap` the way `poll-loop` does — a `PrSnapshot`
      whose key is derived from a **mixed-case** epic-body repo (e.g. `Painworth/Doc-Intel`)
      via `snapshotKey`.
- [ ] The same test drives `processEventBlock` with a webhook payload whose `repo` carries a
      **different casing** (GitHub-canonical, e.g. `painworth/doc-intel`) for a `pr-checks`
      (and separately a `completed:validate` `label-change`) event.
...
- [ ] Mutation check: replacing `snapshotKey(ev.repo, 'pr', ev.number)` at `smee-source.ts:375`
      with an inlined `` `${ev.repo}#pr#${ev.number}` `` makes the new test fail.

## Functional Requirements

| ID | Requirement | Priority | Notes |
|----|-------------|----------|-------|
| FR-001 | Add an integration-level test that drives `SmeeDoorbellSource.processEventBlock` (via the existing test harness in `smee-source.integration.test.ts`) with a `prev` cache populated using a mixed-case epic-body repo and a payload carrying a different (canonical) casing. | P1 | Reuse existing `setPrev` / `fakePrSnapshot` helpers. |
| FR-002 | The test MUST assert the emitted event's `checks` field equals the expected wire value derived from the cached `PrSnapshot.checksRollup`, for both the `pr-checks` event and the `completed:validate` `label-change` event branches at `smee-source.ts:369–373`. | P1 | Both branches share the read-through lookup. |
| FR-003 | The write-side key MUST be constructed through `snapshotKey` (poll-loop-style), and the read-side payload casing MUST differ from the write-side casing, so an inlined lookup that skips normalization produces a cache miss. | P1 | The casing divergence is the load-bearing part. |
