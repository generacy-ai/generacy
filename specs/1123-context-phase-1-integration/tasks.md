...
- [X] T001 Confirm **#1121** (phase machinery) is merged to `develop`: `WorkflowPhase` includes
...
- [X] T002 Confirm **#1122** (per-workflow config) is merged to `develop`: `maxRemediations`
...
- [X] T003 Rebase this branch on `origin/develop` (`git fetch origin && git rebase origin/develop`)
  and verify the union expanded:
  `grep -n "type WorkflowPhase" packages/orchestrator/src/worker/types.ts` shows `review` +
  `remediate`.
- [X] T004 [P] Verify build + baseline suite green post-rebase:
...
- [X] T005 Study the existing harness pattern in
...
- [X] T006 [US1] Create the integration test file and set up a **stub** review/remediate executor
...
- [X] T007 [US1] Assert for **`speckit-feature`**: the loop schedules `review` immediately after
  `implement` (FR-002, SC-002).
- [X] T008 [US1] Assert for **`speckit-bugfix`**: the loop schedules `review` immediately after
  `implement` (FR-002, SC-002).
- [X] T009 [US1] Assert `remediate` is reachable **off the linear sequence** via `{ next: 'remediate' }`
  ...
  to review).
- [X] T010 [US1] Populate `OrchestratorSettings` with feature=3 / bugfix=2 `maxRemediations` + a
...
- [X] T011 [US1] Assert **negative scope** (FR-008): no real review/remediation behavior, no PR
...
- [X] T012 [US2] Create the pause/resume test file. Drive
...
- [X] T013 [US2] Same round-trip for `phase: 'remediate'` — assert resume resolves back to
  `remediate` (re-enter the remediation step, Q3=A — NOT `review`) (FR-005, SC-004).
- [X] T014 [US2] Assert `labelManager` applies then clears the `waiting-for:*` / `phase:*` / `agent:*`
...
- [X] T015 [US3] Establish the runtime source-of-truth phase keyset for the audit (a TS union has no
...
- [X] T016 [US3] Extend `types.test.ts`: assert every `WorkflowPhase` appears in `PHASE_SEQUENCE`
...
- [X] T017 [US3] Assert `pause-context.ts` `WorkflowPhaseSchema.options` **set-equals** the keyset,
...
- [X] T018 [US3] Mutation check (SC-005): by hand during review, drop `review`/`remediate` from one
...
- [X] T019 Verify/finalize the shipped contract at
...
- [X] T020 Run all three suites green (SC-001):
