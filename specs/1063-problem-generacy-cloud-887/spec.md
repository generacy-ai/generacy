...
|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|-------|
| FR-001 | Add a `cluster.cockpit.reply` member to `RelayMessageSchema` in `packages/cluster-relay/src/messages.ts` mirroring cloud's `ClusterCockpitReplyMessage` shape. Fields `frameType`, `wroteDoc`, and `reason` are typed as open `z.string()` (not closed enums), with the currently-known values documented in a code comment (Q2=A). | P1       | Fields: `type` (literal), `timestamp`, `frameId: string \| null`, `frameType: z.string()` (known: `'gate-open'`, `'gate-outcome'`, `'unknown'`), `gateId`, `gateKey?`, `accepted: boolean`, `reason?: z.string()`, `priorStatus?`, `wroteDoc?: z.string()` (known: `'created'`, `'rebound'`). |
| FR-002 | The new schema is tolerant of unknown top-level fields using Zod `.passthrough()` — extras are **preserved** (not stripped) so cloud-added fields land in the router's log lines automatically (Q1=A). No closed enums on `frameType`, `wroteDoc`, or `reason` (Q2=A). | P1       | Prevents the same drop-and-warn regression on future cloud extensions, and preserves unknown fields for observability rather than silently discarding them. |
| FR-003 | The relay message router in `relay.ts` recognises the new message type and does not fall through to the `Invalid relay message, skipping` warn branch. After logging, the router **short-circuits** (returns) — mirroring the existing `api_request` pattern — so the message never reaches `messageHandlers` fanout (Q3=A). | P1       | Structural enforcement of "observability-only" (FR-006). A stray handler cannot begin correlating before #1059 steps 4–7 are designed. |
| FR-004 | On `accepted: true`, the router logs the **full parsed message object** at `debug` and drops (Q4=C). This makes the `.passthrough()` field preservation from FR-002 actually visible on the happy path. | P1       | No downstream consumer; correlation deferred to #1059 steps 4–7. Chosen over a symmetric FR-005 field-set because `reason` and `priorStatus` are undefined on `accepted: true`, and `debug` is opt-in so volume is acceptable. |
