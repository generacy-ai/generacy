...
**Answer**: A) Pause with a `waiting-for:*` gate + `agent:paused` (recoverable; symmetric with the existing timeout pause). Rationale: US1/FR-002 mandate a recoverable, operator-visible paused state with no terminal `blocked:*`; a `failed:validate`-class escalation reads as terminal and bounded re-checks drift into out-of-scope auto-remediation.
...
**Answer**: A) Reuse `waiting-for:ci` (minimal surface; red-CI vs timeout disambiguated by the reason comment). Rationale: the Summary's example is "`waiting-for:ci` plus a red-CI reason comment" and FR-002 wants symmetry with the timeout pause; reuse keeps the single `GATE_MAPPING['ci']` resume path needed by the Q3 re-run-`validate` recovery.
...
**Answer**: A) Re-run `validate` on resume — re-marks ready, re-waits CI on the new SHA, re-evaluates the merge gate. Rationale: the P0 is a passing narrow `validate` co-existing with red CI; resuming straight to completion trusts the operator's word and re-opens the silent-red-complete hole (SC-001/SC-002).
...
**Answer**: A) Same pause state as red CI (reuse the Q1/Q2 gate + label), differentiated only by the reason comment/log. Rationale: Assumptions state the head-SHA fast-fail surfaces as the same class as the red-CI pause; the distinct reason comment/log already names the SHA-resolution failure (FR-004/US2) without a separate label.
...
**Answer**: C) Conservative middle — guard only when the token is known to lack `checks:read` (fallback actually used), document otherwise. Rationale: Assumptions say FR-007 may ship as documentation + a conservative guard; a blanket fail-closed guard would over-trigger on healthy `checks:read` clusters (risking FR-008), while doc-only leaves the false-green live.
