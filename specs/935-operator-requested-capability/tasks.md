...
- [X] **T001** [P] [CH4] Extend `ParsedEpicBody` type in `packages/cockpit/src/resolver/types.ts`
  - Add `adhocRefs: IssueRef[]` field to `ParsedEpicBody` interface.
  - Update JSDoc per data-model.md §Modified types.

- [X] **T002** [P] [CH4] Extend `parseEpicBody` in `packages/cockpit/src/resolver/parse-epic-body.ts`
...
- [X] **T003** [P] [CH4] Extend parser tests in `packages/cockpit/src/resolver/__tests__/parse-epic-body.test.ts`
...
- [X] **T004** [P] [CH4] Relax `resolveEpic` in `packages/cockpit/src/resolver/resolve.ts`
...
- [X] **T005** [P] [CH4] Extend resolver tests in `packages/cockpit/src/resolver/__tests__/resolve.test.ts`
...
- [X] **T006** [P] [CH2] Add `updateIssueBody` to `GhWrapper` in `packages/cockpit/src/gh/wrapper.ts`
...
- [X] **T007** [CH2] Implement `applyScopeMutation` in `packages/generacy/src/cli/commands/cockpit/scope/writer.ts` (NEW)
...
- [X] **T008** [P] [CH2] Writer tests in `packages/generacy/src/cli/commands/cockpit/scope/__tests__/writer.test.ts` (NEW)
...
- [X] **T009** [P] [CH2] Define `ScopeContendedError` in `packages/generacy/src/cli/commands/cockpit/scope/errors.ts` (NEW)
...
- [X] **T010** [CH2] Implement `writeScopeWithRetry` in `packages/generacy/src/cli/commands/cockpit/scope/retry.ts` (NEW)
...
- [X] **T011** [P] [CH2] Retry tests in `packages/generacy/src/cli/commands/cockpit/scope/__tests__/retry.test.ts` (NEW)
...
- [X] **T012** [CH2] Implement `scopeCommand()` in `packages/generacy/src/cli/commands/cockpit/scope.ts` (NEW)
...
- [X] **T013** [CH2] Register `scopeCommand()` in `packages/generacy/src/cli/commands/cockpit/index.ts`
  - `command.addCommand(scopeCommand())` after `resumeCommand()` (registration order per contracts/cli-verbs.md).

- [X] **T014** [P] [CH2] CLI tests in `packages/generacy/src/cli/commands/cockpit/__tests__/scope.test.ts` (NEW)
...
- [X] **T015** [P] [CH3] Add `--issue` form to `packages/generacy/src/cli/commands/cockpit/queue.ts`
...
- [X] **T016** [P] [CH3] Extend queue tests in `packages/generacy/src/cli/commands/cockpit/__tests__/queue.test.ts`
...
- [X] **T017** [P] [CH1] Modify `computeTransitions` in `packages/generacy/src/cli/commands/cockpit/watch/diff.ts`
...
- [X] **T018** [P] [CH1] Extend diff tests in `packages/generacy/src/cli/commands/cockpit/watch/__tests__/diff.test.ts`
...
- [X] **T019** [P] [CH4] Add flat-mode render to `packages/generacy/src/cli/commands/cockpit/status.ts`
...
- [X] **T020** [P] [CH4] Add adhoc group to `packages/generacy/src/cli/commands/cockpit/status/group.ts`
  - Include adhoc refs as a distinct group with heading `Ad-hoc` in phased-body responses when non-empty.

- [X] **T021** [P] [CH4] Extend status tests in `packages/generacy/src/cli/commands/cockpit/__tests__/status.test.ts`
...
- [X] **T022** [CH2] Add scope schemas + modify queue schema in `packages/generacy/src/cli/commands/cockpit/mcp/schemas.ts`
...
- [X] **T023** [P] [CH2] Add error classes to `packages/generacy/src/cli/commands/cockpit/mcp/errors.ts`
  - `'contended'` (maps to `SCOPE_ADD_CONTENDED`).
  - `'scope-not-found'` (maps to scope-issue 404).
  - Reuse existing classes for `invalid-args` etc.

- [X] **T024** [CH2] Implement `cockpit_scope_add` handler in `packages/generacy/src/cli/commands/cockpit/mcp/tools/cockpit_scope_add.ts` (NEW)
...
- [X] **T025** [CH2] Implement `cockpit_scope_remove` handler in `packages/generacy/src/cli/commands/cockpit/mcp/tools/cockpit_scope_remove.ts` (NEW)
...
- [X] **T026** [CH3] Modify `cockpit_queue` handler in `packages/generacy/src/cli/commands/cockpit/mcp/tools/cockpit_queue.ts`
...
- [X] **T027** [CH2] Register scope tools in `packages/generacy/src/cli/commands/cockpit/mcp/server.ts`
...
- [X] **T028** [P] [CH2] Parity test in `packages/generacy/src/cli/commands/cockpit/mcp/__tests__/parity-scope-add.test.ts` (NEW)
...
- [X] **T029** [P] [CH2] Parity test in `packages/generacy/src/cli/commands/cockpit/mcp/__tests__/parity-scope-remove.test.ts` (NEW)
  - MCP-form `cockpit_scope_remove` parity with CLI `scope remove`.
  - Idempotent (already-absent) return.

- [X] **T030** [P] [CH3] Extend parity test in `packages/generacy/src/cli/commands/cockpit/mcp/__tests__/parity-queue.test.ts`
...
- [X] **T031** [P] [CH5] Registry isolation test in `packages/generacy/src/cli/commands/cockpit/mcp/__tests__/registry-isolation.test.ts` (NEW)
...
- [X] **T032** [P] [CH1] Registry liveness test in `packages/generacy/src/cli/commands/cockpit/mcp/__tests__/registry-liveness.test.ts` (NEW)
