...
| FR-002 | `RedisQueueAdapter.release()` MUST perform the read-and-re-pend of its **retry** branch in a single server-side atomic operation.                                    | P1       | Dead-letter branch may remain a subsequent `MULTI` (or fold into the script), but MUST preserve the `SREM` of the in-flight SET. |
...
| FR-005 | Both scripts MUST return an unambiguous outcome code so the caller can log the correct branch: `no-op (already-cleared)` vs `re-pended` (and, for `release`, `dead-lettered`). | P1       | Mirrors `RECLAIM_ORPHAN_SCRIPT` return codes. |
...
|--------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------|-------------|
| SC-001 | Regression test that runs `reapOrphanClaims` and `requeueForResume` concurrently against a real Redis instance leaves `ZCARD orchestrator:queue:pending == 1`.                        | 100% pass (0 failing runs across ≥100). | New integration test in `packages/orchestrator/src/services/__tests__/`. |
| SC-002 | Same test for `release()` (retry branch) also leaves `ZCARD == 1`.                                                                                                                    | 100% pass.                              | New integration test. |
| SC-003 | `requeueForResume()` end-to-end operation issues exactly **one** Redis round trip (excluding the null-guard `DEL heartbeat` on the reaper-wins path).                                  | 1 round trip on happy path.             | `MONITOR`-based test or command-count assertion. |
| SC-004 | `release()` end-to-end operation issues exactly **one** Redis round trip on both the retry branch and the dead-letter branch (excluding the null-guard).                              | 1 round trip on happy path.             | `MONITOR`-based test or command-count assertion. |
...
- **A5**: The script MUST re-serialize the re-pend payload client-side (`JSON.stringify`) before passing to the script, matching `RECLAIM_ORPHAN_SCRIPT`'s `ARGV[5]` convention — because Lua `cjson.encode` does not guarantee stable key ordering, and downstream code path may key on member-string identity.
