...
**Acceptance Criteria**:
- [ ] When `enqueueIfAbsent` drops an item because it is already in-flight, the log line's severity is conditional on the age of the in-flight entry: at `info` when the entry is younger than the maximum plausible run duration, and at `warn` (or higher) when it has exceeded it. The "age" is derived from the claim hash's `enqueuedAt` field (which is present on every `SerializedQueueItem`).
...
- [ ] The four monitor-side "Dropping ... enqueue (item already in flight)" sites (PR-feedback, merge-conflict, clarification-answer, label-resume) inherit the same escalation — either by delegating to a shared helper or by matching the same age-vs-threshold decision. Divergence between the adapter-level drop and the monitor-side context line is a regression.
...
| FR-002 | On reclaim, the reaper must (a) `SREM orchestrator:queue:in-flight-items <itemKey>`, (b) re-enqueue the item to `orchestrator:queue:pending` with `queueReason: 'resume'` and its original `enqueuedAt` and `metadata` preserved from the claim hash payload, (c) `HDEL orchestrator:queue:claimed:<workerId> <itemKey>` and `DEL orchestrator:queue:claimed:<workerId>` if the hash is now empty. Steps (a) + (b) + the HDEL/DEL must be co-atomic in a single `MULTI`/`EXEC` so a concurrent live-worker `complete()` cannot race the reclaim into a double-enqueue. | P1 | Preserves resume-priority semantics used by `handleLeaseExpired` (`worker-dispatcher.ts:267-273`). Atomicity is load-bearing — the reclaim must not be observable as "not-in-flight AND not-in-pending" for any window. |
...
| FR-004 | Before writing the reclaim (FR-002), the reaper must re-check that the heartbeat key is still absent — the check is issued *twice*: once for the initial iteration filter, once inside the `MULTI` block via a Lua-scripted guard (`WATCH` on the heartbeat key with abort-on-change is the equivalent primitive). A heartbeat that reappears between the two checks aborts the reclaim for that claim. | P1 | US2 race safety. Prevents a live worker's mid-cycle heartbeat blip from causing double-execution. |
| FR-005 | The reaper must NOT reclaim claims whose claim-hash `enqueuedAt` timestamp is within a grace window (default: 2 × `heartbeatCheckIntervalMs`, i.e. 30s at defaults) of `now`. This defends against a race where a worker has just claimed but not yet completed its initial `SET heartbeat` (impossible under `CLAIM_SCRIPT`'s current atomicity, but robust against future refactors that split the two operations). | P2 | Cheap defensive check; costs nothing at steady state. |
| FR-006 | `RedisQueueAdapter.enqueueIfAbsent`'s drop-log line (`redis-queue-adapter.ts:141-144`) must escalate to `warn` when the age of the in-flight entry exceeds the maximum plausible run duration. The age must be read from the claim hash's `enqueuedAt` field (an existing field on `SerializedQueueItem`). Threshold: single configurable value (default derived from `shutdownTimeoutMs × 3` or an explicit `maxRunDurationMs` — see clarifications). At age below threshold, the line stays at `info` (steady-state dedup log, unchanged). | P1 | US3. The log escalation is the operator-facing surface — an operator paging on `warn`s sees this bug in the first cycle past the threshold, not by re-reading Redis by hand. Independent of FR-001–FR-005 (the reclaim fixes the state; this makes the intermediate window audible). |
| FR-007 | The four monitor-side "Dropping … enqueue (item already in flight)" context lines (`pr-feedback-monitor-service.ts:428`, `merge-conflict-monitor-service.ts:186`, `clarification-answer-monitor-service.ts:240`, `label-monitor-service.ts:361`) must escalate to `warn` on the same age-vs-threshold rule as FR-006. The two-log-lines-per-drop pattern is preserved — the second (monitor-side) line adds context (`prNumber`, `phase`, etc.) that the adapter doesn't have. | P1 | Preserves the pattern the codebase already established (adapter emits generic drop; monitor emits context-rich drop). Divergent severity between the two would confuse alerting. |
| FR-008 | The reclaim must emit a `warn`-level log line (or higher) naming the workerId, the itemKey, the age of the claim, and the reason (`orphaned-claim-no-heartbeat`). This is the operator-facing evidence that the fix fired — distinct from FR-006's drop-side warn. | P2 | Complements the in-memory reap's existing `worker-dispatcher.ts:583-586` warn. Makes the two paths differentiable in log queries. |
...
| FR-011 | The fix must work with both Redis (`RedisQueueAdapter`) and in-memory (`InMemoryQueueAdapter`) queue adapters. The in-memory adapter has an equivalent shape (its own drop log at `in-memory-queue-adapter.ts:89`) — its reclaim path may be a no-op if it has no equivalent orphan case (verify during `/plan`), but the log escalation from FR-006 must apply uniformly. | P2 | The two adapters share the `QueueManager` interface; keeping behaviour parity is a maintenance property, not a runtime necessity in prod (prod runs Redis). |
| FR-012 | The fix must not introduce a new configurable knob without a documented default. If a `maxRunDurationMs` or equivalent is added to `DispatchConfigSchema`, its default must be derivable from existing signals (e.g., `shutdownTimeoutMs × N`, or the observed CLI timeout of 20 minutes = 1200000ms) and documented alongside it. | P2 | The one-knob-fits-all threshold is a rough heuristic; over-tuning is out of scope for this fix. |
...
- The maximum plausible run duration used to gate the log escalation (FR-006/FR-007) is significantly larger than steady-state healthy dispatch. Typical happy-path handler completes in seconds to a few minutes; the pathological case observed in this incident was 20 minutes (CLI timeout). A default threshold of 20-30 minutes catches wedges without producing `warn` spam on legitimately-long runs.
