...
**Acceptance Criteria**:
- [ ] When `remediate` executes, the CLI is spawned with an in-process prompt built from the open blocking findings in the review artifact (and/or validate-failure evidence).
...
| FR-002 | Build the remediation prompt in-process from the open blocking findings in the review artifact (`status: open` at/above the profile's `blockingSeverity`) and/or validate-failure evidence. No new slash command; `PHASE_TO_COMMAND` is not extended. | P1 | Mirrors #1124 FR-002 charter-prompt approach. |
...
| FR-005 | Maintain an explicit remediation counter: one `remediate` execution increments it by exactly one, regardless of finding count. Persist it in the review sidecar. | P1 | Distinct from review `round` because it is reset on resume (FR-009). |
...
| FR-007 | At the cap, raise the gate `waiting-for:remediation-limit` + `agent:paused` and pause the workflow. No terminal `blocked:*` label anywhere in this path. | P1 | The `on-remediation-limit` gate condition already exists (`phase-loop.ts:1122`); this re-keys it to the remediation counter. |
...
| FR-009 | An operator answer of `completed:remediation-limit` resumes into the remediate loop and resets the remediation counter to a fresh budget. | P1 | Honors the `waiting-for:X` → `completed:X` satisfaction convention. |
| FR-010 | Add/adjust `GATE_MAPPING` / `WORKFLOW_GATE_MAPPING` entries in `worker/phase-resolver.ts` so a `completed:remediation-limit` resume routes into the remediate loop. | P1 | Current entry maps `remediation-limit` → `{ phase: review, resumeFrom: review }`; confirm/adjust so it resumes into remediation. |
...
- The relationship between the review `round` and the resettable remediation counter (same field vs. distinct field, and whether `round` is also reset on resume) is a design detail to settle in `/clarify` / `/plan` — the requirement is that the *cap-driving* counter resets on resume while the loop still converges.
