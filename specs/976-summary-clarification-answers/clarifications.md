...
**Answer**: A — marker-based machine exclusion only; no cluster config, no identity classification (zero operator setup). Every cluster-emitted machine comment already carries a marker, so the scanner simply broadens its exclusion set from question-markers to ALL machine-marker families. Apply at BOTH call sites: the monitor (`clarification-answer-monitor-service.ts:202`) and the phase-loop scanner (`clarification-poster.ts:918-931`).
...
**Answer**: A — all three types already carry markers, so this is scanner-side only, no poster migration. Verified: questions=`generacy-clarifications:`/`generacy-stage:clarification`, stage/status=`generacy-stage:specification|planning|implementation`, audit=`generacy-cockpit:manual-advance`. Implementation note: today the scanner excludes only `CLARIFICATION_QUESTION_MARKERS`; broaden it to one canonical `MACHINE_MARKERS` set covering every family (also `generacy-clarification-answers:`, `generacy-untrusted-answer:`, `generacy-clarification-parse-failures:`, and the `speckit-stage:*` variants).
...
**Answer**: A — reuse the existing untrusted-answer explainer comment path (`generacy-untrusted-answer:` family / `postUntrustedAnswerExplainers`). Post an explainer comment describing why the answer was rejected; no new label. This directly removes the silent re-arm that made the original bug hard to diagnose.
...
**Answer**: A — parity with the different-account permissive path (FR-006): any trusted, marker-free comment is a candidate, and `Q<n>:` parsing decides what actually integrates. No extra shape or temporal gate for same-account — safe because machine comments carry markers and a non-answer comment integrates nothing.
...
**Answer**: D — not applicable. Q1=A uses no identity signal, so there is no cluster-login human/bot classification to perform.
