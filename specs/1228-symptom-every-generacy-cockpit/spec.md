...
**Acceptance Criteria**:
- [ ] A fresh (missing/stale) cursor against a pre-existing file degrades safely and does
      not replay the whole file. [NEEDS CLARIFICATION: exact degrade policy — replay
      nothing vs. bound by `answeredAt` recency window]
...
| FR-002 | Drop foreign-epic answers (same repo or cross repo) and log them at `info` with the `gateId`. | P1 | |
...
| FR-004 | Persist the consumed position (ino + byte offset) per epic scope so replay resumes from the last consumed byte across process restart, rotation, and truncation. | P1 | The ack/consumed-position half never implemented in #1023. |
| FR-005 | On a missing or stale cursor, degrade safely — do not replay the entire pre-existing file. | P1 | Degrade policy to be settled in clarify (see US4). |
...
| FR-008 | Unit tests cover: fresh-cursor, resumed-cursor, rotation, truncation, same-repo-foreign-epic drop, cross-repo in-scope emit. | P1 | |
...
- The bound epic's resolved ref set is available to `AnswersFileSource` the same way it is
  ...
  path.
- The cursor store may live alongside the answers file under `.generacy/cockpit/`; exact
  location/format to be decided in plan. [NEEDS CLARIFICATION: cursor persistence location
  and format]
