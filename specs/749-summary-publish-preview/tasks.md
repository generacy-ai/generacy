...
- [ ] T002 [P] [US1] Create `scripts/check-preview-staleness.mjs` — Node ESM, `node:` builtins only. Reads `candidateSha` via `git rev-parse HEAD`. Reads `currentPreviewSha` via `npm view @generacy-ai/generacy@preview gitHead` (anchor package from D7). Decision table from data-model.md §"State transitions":
