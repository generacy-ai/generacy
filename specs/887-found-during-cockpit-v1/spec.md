...
| FR-003 | The Zod schema surface MUST expose a single discriminated union on `type` covering both per-issue and aggregate events. | P1 | Consumers import one union, not two. |
| FR-004 | The `type` field MUST be set in exactly one place per emit path (no fan-out of literal strings across call sites). | P2 | Single source of truth for discriminator. |
...
| FR-006 | No fields on per-issue or aggregate lines may be removed or renamed by this change. | P1 | Purely additive. |
| FR-007 | The package README MUST include a stream-grammar table enumerating every `type` value, its full field set, and the invariant "every emitted line is a JSON object with a `type` field". | P1 | Single documented contract. |
| FR-008 | A regression test MUST assert the README table's enumerated `type` set matches the discriminated union's members (drift check). | P1 | Same pattern as gate-vocabulary audit. |
| FR-009 | A regression test MUST assert every emit call site in the package produces a line that parses against the discriminated union. | P1 | Prevents future emitters that skip the discriminator. |
...
- The single-place-of-truth for the `type` literal on per-issue events is `emit.ts`'s `CockpitEventSchema` + the `emit()` function.
