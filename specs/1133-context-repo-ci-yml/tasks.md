...
- [X] T001 [US1] Add `CiConclusion`, `CiRun`, `CiVerdict` types to
...
- [X] T002 [US1] Implement pure `aggregateCiVerdict(runs: CiRun[]): CiVerdict` in NEW file
...
- [X] T003 [P] [US1] Unit test `packages/workflow-engine/src/actions/github/client/__tests__/ci-verdict.test.ts`
...
- [X] T004 [US1] Add `getCiRunsForSha(owner, repo, headSha, branch): Promise<{ runs: CiRun[]; source: 'check-runs' | 'actions-runs' }>`
  to the `GitHubClient` interface in
  `packages/workflow-engine/src/actions/github/client/interface.ts` (depends on T001).
- [X] T005 [US1] Implement `getCiRunsForSha` in
  ...
  (Depends on T004.)
- [X] T006 [P] [US1] Unit test `packages/workflow-engine/src/actions/github/client/__tests__/gh-cli.ci-readout.test.ts`:
...
- [X] T007 [US2] Create NEW file `packages/orchestrator/src/worker/ci-merge-readiness.ts`:
...
- [X] T008 [P] [US2] Unit test `packages/orchestrator/src/worker/__tests__/ci-merge-readiness.test.ts`:
...
- [X] T009 [US3] Add `waiting-for:ci` (color `FBCA04`) and `completed:ci` (color `0E8A16`) to
  `packages/workflow-engine/src/actions/github/label-definitions.ts` per contracts/labels.md.
- [X] T010 [US3] In `packages/orchestrator/src/worker/config.ts`:
...
- [X] T011 [US3] In `packages/orchestrator/src/config/loader.ts` read `WORKER_CI_MERGE_GATE_ENABLED`
...
- [X] T012 [US3] In `packages/orchestrator/src/worker/phase-resolver.ts` thread `ciMergeGateEnabled`
...
- [X] T013 [P] [US3] Resolver test `packages/orchestrator/src/worker/__tests__/phase-resolver.ci-merge.test.ts`:
...
- [X] T014 [US3] In `packages/orchestrator/src/worker/phase-loop.ts` (flag-gated):
...
- [X] T015 [US3] In `packages/orchestrator/src/worker/claude-cli-worker.ts` pass
  `this.config.ciMergeGateEnabled` into `phaseResolver.resolveStartPhase(...)`.
- [X] T016 [P] [US3] Integration test
...
- [X] T017 [P] [US4] Add a migration note (in `specs/1133-context-repo-ci-yml/quickstart.md` and/or the
...
- [X] T018 Add changeset `.changeset/1133-ci-merge-gate.md`:
...
- [X] T019 Run `pnpm -w build`, `pnpm -w lint`, and the touched packages' `vitest` suites
