...
**Answer**: C — Instrumented stub runner that records each actual test/build suite spawn at the runner seam; suite-execution count observed there.
...
**Answer**: A — Synthetic, self-contained fixture checked into the test tree with a hand-authored dependency graph guaranteeing >=1 package with no dependents, so changed+dependents is provably a strict subset.
...
**Answer**: A — Validate seam is a stub keyed on (command, ref) returning injected pass/fail per invocation; seed fail for the base-ref run and pass for the branch run.
...
**Answer**: A — Inject CI status via the merge-readiness dependency seam; author a green-both-pass scenario AND explicit negative scenarios for skipped and neutral (each asserting the final gate is NOT raised).
...
**Answer**: A — Docs example lives in a repo docs location (docs/ or package README) AND is asserted valid by a test parsing it against the shipped P4 config schema (no drift).
