...
- A **monorepo fixture** with multiple packages and a real dependency graph (so `pnpm --filter "...[origin/<base>]"` resolves a proper *changed-plus-dependents* set) is authored as part of the harness; the affected-set-vs-full-workspace suite-count comparison is meaningful only on such a fixture. The fixture is a **synthetic, self-contained fixture checked into the test tree** with a hand-authored dependency graph that guarantees at least one package with no dependents, so changed+dependents is provably a strict subset. (Clarification Q2 → A.)
- **Suite-execution count** is measured via an **instrumented stub runner** that records each actual test/build suite spawn it is asked to execute; the count is observed at that runner seam (not by counting `--filter`-resolved package targets, not by counting validate-command invocations, and not by executing real 10-minute suites). (Clarification Q1 → C.)
- CI is external and its status is **injected via the merge-readiness dependency seam** (#1133); the harness does not run real GitHub Actions. Validate/CI "parallelism" is asserted as the engine's sequencing/readiness behavior, not wall-clock concurrency. The suite authors a green-both-pass scenario AND explicit negative scenarios for `skipped` and `neutral` CI, each asserting the final gate is NOT raised. (Clarification Q4 → A.)
- The `failThenPass` fail-on-base / pass-on-branch outcome is simulated through a **validate seam stubbed on (command, ref)** that returns injected pass/fail per invocation; the scenario seeds "fail" for the base-ref run and "pass" for the branch run. (Clarification Q3 → A.)
- The docs bugfix-profile config example lives in a **repo docs location** (e.g. `docs/` or a package README) AND is asserted valid by a test that parses it against the shipped P4 config schema, so it cannot silently drift. (Clarification Q5 → A.)
