...
**Answer**: A — `specs/<feature>/stack.md`. The branch already owns `specs/<feature>/` exclusively, so disjointness is by construction; co-location with `spec.md`/`plan.md` is where both humans and agents already look. B mints a second managed root for no gain.
...
**Answer**: A — leave-alone, because it is the only option that adds zero new CLAUDE.md edits. B recreates the disease during migration: two in-flight siblings each stripping the legacy region are two branches editing CLAUDE.md again — conflicting strips. C is worse: a base-side strip makes every in-flight branch's next base-merge a delete-vs-modify conflict on exactly the region we're escaping. With A, new branches simply stop touching the file, in-flight legacy sections drain out as branches merge, and base's accumulated prose gets cleaned manually in a quiet moment, off the critical path.
...
**Answer**: Verify Assumption 2 before choosing — it is likely wrong, which dissolves the question. The speckit operations live in *this repo* (`packages/workflow-engine/src/actions/builtin/speckit/operations/` — precedent: #842 modified clarify behavior right there). If the CLAUDE.md append originates in this repo's plan operation/template, ship everything here and strike the assumption. Only if the write genuinely comes from an externally-owned skill does A apply — companion issue in the owning repo, and **no local-override shims** (B/C): that's drift machinery bridging a gap measured in days on our own pipeline.
...
**Answer**: A — whole-file overwrite; the file exists solely as `/plan`'s output, so full ownership with a "generated by /plan — do not hand-edit" header is the honest contract. Marker regions inside a fully-generated file are ceremony; humans who want durable notes have `spec.md`/`plan.md` an inch away.
...
**Answer**: A, reframed to test the deterministic layer — an automated in-repo test, but don't run two full agent-driven `/plan` invocations in CI (expensive, flaky): unit-test the managed-file writer (or statically assert the plan template contains no CLAUDE.md-write instruction and targets the per-feature path), then simulate two sibling writes and assert `git merge-tree` reports zero conflicts. The full two-branch agent flow stays a T-S* smoke-test checklist item, where it is already exercised for free.
