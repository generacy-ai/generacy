...
- [ ] T001 [US4] Add `ciWaitTimeoutMs: z.number().int().min(30_000).optional()` to the
...
- [ ] T002 [US4] In `packages/orchestrator/src/worker/config.ts`: add
...
- [ ] T003 [P] [US3] Add pure helper `resolveReviewLikeAgent(config, workflowName, phase:
...
- [ ] T004 [US1] In `packages/orchestrator/src/worker/phase-loop.ts`, change the
...
- [ ] T005 [US2] In `packages/orchestrator/src/worker/phase-loop.ts`, replace the raw
...
- [ ] T006 [US4] In `packages/orchestrator/src/worker/phase-loop.ts`, wire the resolved
...
- [ ] T007 [US3] In `packages/orchestrator/src/worker/review-executor.ts` (~line 126),
  replace `resolveAgentForPhase(config, w, 'implement')` with
  `resolveReviewLikeAgent(config, w, 'review')`. Depends on T003.

- [ ] T008 [US3] In `packages/orchestrator/src/worker/remediate-executor.ts` (~line 98),
  replace `resolveAgentForPhase(config, w, 'implement')` with
...
- [ ] T009 [P] [US4] MOD `packages/config/src/__tests__/template-schema.test.ts`: assert
...
- [ ] T015 NEW `.changeset/1160-config-keys-apply.md`: `@generacy-ai/config` **minor**
