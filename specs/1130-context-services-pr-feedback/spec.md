...
| FR-003 | A trusted external unresolved thread MUST cause worker re-entry into the shared review/remediate loop, not the legacy fixer. Entry mechanism: the monitor synthesizes the findings artifact, then enqueues a `review`-phase re-entry; the existing review→remediate seam takes over (verdict = changes-required → `remediateTrigger` fires). | P1 | One loop, one counter, one code path. Clarified (Q1→A). Re-entry at `review` (never directly at `remediate`), matching how the `remediation-limit` gate resumes. |
| FR-004 | External feedback findings MUST be synthesized into the findings artifact from BOTH inline threads AND review bodies. Synthesis reuses the legacy fixer's dual-source parser (`pr-feedback-handler.ts:249-360`) as the extraction step, feeding results into the artifact worker-side (where the checkout + trust config live). | P1 | Preserve dual-source behavior. Clarified (Q2→C). Monitor has no checkout and cannot write the checkout-local sidecar. |
...
| FR-006 | A new human review/comment submitted after the cap is hit MUST reset the remediation counter (new external feedback = fresh budget). Reset is authorship-based, never content-based; full thread resolution and gate-label removal alone do NOT reset it. | P1 | Clarified (Q3→A). Distinct from #1070's `fixerTimeoutRetryCount`. |
| FR-007 | The legacy `pr-feedback-handler` MUST be reduced to a thin adapter that synthesizes findings + enqueues remediate, deleting the divergent dispositions (notably `blocked:stuck-feedback-loop`). Its dual-source extraction (FR-004) is retained and reused. | P2 | No loss of behavior coverage. Clarified (Q5→B) — not full deletion (avoids re-homing the parser and dropping body-only findings) and not keep-in-place (avoids a second live fix path). |
...
| FR-010 | When an unresolved thread mixes engine-authored and trusted-external comments, the thread MUST still trigger on the external comment(s). A thread is excluded ONLY when ALL its comments are engine-authored; any trusted-external comment keeps it live. | P1 | Clarified (Q4→A). |
