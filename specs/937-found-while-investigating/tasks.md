...
- [X] T010 [P] [US1][US2] Run unit tests: `pnpm --filter @generacy-ai/orchestrator test post-activation` and `pnpm --filter @generacy-ai/control-plane test lifecycle`. All RT-001..RT-004 must pass. Full-package runs (`pnpm --filter @generacy-ai/orchestrator test`, `pnpm --filter @generacy-ai/control-plane test`) must remain green (no unrelated regressions).
- [X] T011 [P] [US1] Typecheck the two touched packages (`pnpm --filter @generacy-ai/orchestrator typecheck && pnpm --filter @generacy-ai/control-plane typecheck`). Confirms no downstream consumer of `PostActivationRetryOptions` or the lifecycle response body breaks.
