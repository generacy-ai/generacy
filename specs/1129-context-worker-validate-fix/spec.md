...
|----|-------------|----------|-------|
| FR-001 | On `validate` failure, capture failure evidence using the existing evidence mechanics (`result.capturedStdout` / `capturedStderr` / `exitCode`) and enter the `remediate` phase. | P1 | Reuses evidence already assembled in the validate `catch` block. |
| FR-002 | The validate-triggered remediation counts against the per-workflow `maxRemediations` counter — the same counter used by review-driven remediations. | P1 | No separate budget for validate. |
...
| FR-005 | Retire `worker/validate-fix-handler.ts` or reduce it to a thin adapter; the one-attempt-per-evidence-hash cap is superseded by `maxRemediations`. | P1 | Remove `blocked:stuck-validate-fix` as the primary termination if the handler is fully retired. |
...
| FR-009 | When the remediation budget is exhausted for a validate-driven loop, pause via the `on-remediation-limit` gate (consistent with review-driven exhaustion). | P2 | Reuses `phase-loop.ts:1122-1145`. |
