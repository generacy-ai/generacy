...
- [ ] T002 [P] [US1] Widen `ChecksRollup` in `packages/generacy/src/cli/commands/cockpit/watch/snapshot.ts` from `'pending' | 'success' | 'failure'` to `'pending' | 'success' | 'failure' | 'none' | 'error'`. Contract: `contracts/checks-rollup-union.md` §Union declaration.
- [ ] T003 [P] [US1] Widen `StatusRow.checks` in `packages/generacy/src/cli/commands/cockpit/status/row.ts` from `'pending' | 'success' | 'failure' | 'none'` to `'pending' | 'success' | 'failure' | 'none' | 'error'`. Widen the matching `checks` parameter type on `buildStatusRow(...)`. Body unchanged.
