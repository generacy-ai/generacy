...
- [ ] Worker logs and gateway access logs from the scaffolded cluster match the tetrad dev run (gateway-route models hit the gateway; subscription-route models do not).
...
| FR-002 | Compare the scaffolded local cluster's worker logs and gateway access logs against the tetrad dev run; attribute any divergence. | P1 | Route discrimination must match |
...
| FR-005 | Diff-check the gateway stanza across scaffolder output, tetrad dev compose, cluster-base compose, and cloud-deploy template; fix in-repo or file against the owning repo. | P1 | Four-way reconciliation |
| FR-006 | Validate `cockpit auto` correctly configures mixed-route agent config on the dogfood run. | P2 | Per issue title |
...
- The mixed-route dogfood recipe from #1203 is the canonical validation workload.
