...
- [X] T006 [US1, US2, US3] Implement the sweep in `packages/generacy/src/cli/commands/cockpit/watch/diff.ts`:
...
- [X] T007 [P] [US1, US2, US3] Run the full watcher test suite from quickstart.md: `pnpm --filter @generacy-ai/generacy test -- watch`. All existing tests pass unmodified (except the renamed baseline test in T005), and the new tests from T003–T005 pass. Confirms SC-004 (no regressions in existing transition semantics).
...
- [X] T009 [US1] Type-check + build: `pnpm --filter @generacy-ai/generacy build`. Confirms `CockpitEvent.initial?: true` propagates through all callers with no type errors, and that no downstream consumer branches on `initial === false` (which would now be a compile error against the literal type).
