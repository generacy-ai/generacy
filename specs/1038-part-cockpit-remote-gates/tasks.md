...
- [ ] T001 [US2] Rewrite `deriveClarificationGeneration` in `packages/cockpit/src/gates/generation.ts` per data-model.md §4:
...
- [ ] T002 [US2] Update re-exports in `packages/cockpit/src/gates/index.ts`:
...
- [ ] T003 [US2] Extend `packages/cockpit/src/gates/__tests__/gates-generation.test.ts` with the **SC-002 parity fixture**:
...
- [ ] T010 [P] [US1] Add `GateQueryRequestMessage` + `GateQueryResponseMessage` interfaces and Zod schemas in `packages/cluster-relay/src/messages.ts` per contracts/gate-query-relay-envelope.md:
...
- [ ] T011 [P] [US1] Re-export the two new interfaces and both schemas from `packages/cluster-relay/src/index.ts` (or the current package entry) so `import { GateQueryRequestMessage } from '@generacy-ai/cluster-relay'` type-narrows without reaching into `messages.js`.

- [ ] T012 [P] [US1] Extend `packages/cluster-relay/src/__tests__/messages.test.ts` (or the equivalent parse-round-trip test file) per contracts/gate-query-relay-envelope.md § Test coverage:
...
- [ ] T020 [US1] Create `packages/orchestrator/src/services/gate-status-query.ts` per contracts/get-cockpit-gates.md § Service contract:
...
- [ ] T021 [US1] Add `packages/orchestrator/src/services/__tests__/gate-status-query.test.ts` covering the eight cases in contracts/get-cockpit-gates.md § Service-level tests:
...
- [ ] T022 [P] [US1] Modify `packages/orchestrator/src/types/relay.ts` (if it re-exports envelope types) to surface `GateQueryRequestMessage` and `GateQueryResponseMessage` from `@generacy-ai/cluster-relay`. If the file only re-exports payloads, no change required — trust the compiler.

- [ ] T023 [US1] Add the `GET /cockpit/gates` handler to `packages/orchestrator/src/routes/cockpit-gates.ts` per contracts/get-cockpit-gates.md § Handler logic:
...
- [ ] T024 [US1] Extend `packages/orchestrator/src/routes/__tests__/cockpit-gates.test.ts` per contracts/get-cockpit-gates.md § Route-level tests:
...
- [ ] T025 [US1] Wire `GateStatusQueryService` into `packages/orchestrator/src/server.ts` per contracts/get-cockpit-gates.md § Wiring:
...
- [ ] T032 [P] [US1] Re-export the two new input schemas from `packages/generacy/src/cli/commands/cockpit/mcp/schemas.ts` (or wherever `CockpitGateOpenInputSchema` is re-exported for MCP `registerTool` sites) so `mcp/server.ts` can import them at registration time.

- [ ] T033 [US1] Create `packages/generacy/src/cli/commands/cockpit/mcp/gates/query-client.ts` — GET-shaped HTTP client per plan.md § R9 and contracts/cockpit_gate_status.md § Retry semantics:
...
- [ ] T034 [US1] Add `packages/generacy/src/cli/commands/cockpit/mcp/gates/__tests__/query-client.test.ts` covering the R10 retry cases:
...
- [ ] T035 [US3] Create `packages/generacy/src/cli/commands/cockpit/mcp/tools/cockpit_gate_status.ts` per contracts/cockpit_gate_status.md:
...
- [ ] T036 [US1] Create `packages/generacy/src/cli/commands/cockpit/mcp/tools/cockpit_gate_list.ts` per contracts/cockpit_gate_list.md:
...
- [ ] T040 [US1] Extend `packages/orchestrator/src/__tests__/cockpit-gates-integration.integration.test.ts` (the #1024 harness) with one status-query scenario per plan.md § R10 § Integration:
...
- [ ] T041 [US1] Add `.changeset/1038-cockpit-gate-status-query.md` per plan.md § R11:
