...
- [ ] T010 [P] [US1] Create `packages/generacy/src/cli/commands/cockpit/merge.ts`. Export `cockpitMergeCommand()` (Commander) and `runMerge({ gh, issue, repo, logger })` (the directly-testable handler). Gate order — **short-circuit on first failure**:
...
- [ ] T011 [P] [US2] Create `packages/generacy/src/cli/commands/cockpit/review-context.ts`. Export `cockpitReviewContextCommand()` (Commander) and `runReviewContext({ gh, issue, repo, logger })`. Flow:
...
- [ ] T012 [US1][US2] Create the two verb test files:
