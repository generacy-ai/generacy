...
| FR-002 | When `waiting-for:manual-validation` is present, the block must not synthesize a partial and must not re-enter implement. | P1 | Authoritative signal — no heuristics involved |
| FR-003 | On that path the loop must terminate as a gate pause (`agent:paused` + the gate label retained), returning `gateHit: true` and `completed: false`. | P1 | Structural precedent: the #1211 dependency-blocked branch |
...
| FR-006 | Manual classification must use layered detection matching the implement prompt: high-confidence `[manual]` marker, then the medium-confidence keyword set (`manual`, `manually`, `hand-test`, `manually verify`). | P1 | Keyword-only detection is provably insufficient (#2723) |
...
3. Because `manual-validation` resumes at `validate` (a *later* phase than `implement`), the label state applied on pause must leave the resumed run resolvable at `validate`. Which existing `LabelManager` primitive achieves that coherently (`onPhaseComplete` before `onGateHit`, versus `onPhaseExecutedWithoutCompletion`) is an open design question for `/speckit:clarify`.
