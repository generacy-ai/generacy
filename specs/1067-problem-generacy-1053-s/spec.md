...
- `CockpitGateListInputSchema` accepts an optional `runId: z.string().min(1).optional()` (for surface parity; the handler drops it before calling the cloud — see FR-003).
- Both schemas remain `.strict()` (the field is *declared*, not tolerated).
- `cloud-gate-query-client.ts` `getGateStatus()` accepts an optional `runId` and forwards it as a `runId=<value>` query-string parameter (camelCase) to the cloud endpoint when present. `listGates()` is **not** widened; the `cockpit_gate_list` handler MUST NOT forward `runId` (deployed cloud contract 400s any list request carrying `runId`).
...
**Acceptance Criteria**:
- A test asserts that for every combination of (`issueRef`, `gateType`, `generation`, `runId?`) supplied to `cockpit_gate_open`, `cockpit_gate_status`, and `cockpit_gate_list`, the `gateKey`/`gateId` derived on each path (write derivation for open, wire-side derivation for status, cloud-returned `entries[].gateId` for list) all match.
- The test covers both the omitted-`runId` (3-tuple) and explicit-`runId` (4-tuple) cases.
- The `cockpit_gate_list` leg validates that inbox entries carry the correct `gateId` for a matching tuple — the surface an operator actually sees.
...
| FR-002 | `CockpitGateListInputSchema` gains `runId: z.string().min(1).optional()`; schema remains `.strict()` and a flat `z.object`. (Widened for surface parity with `cockpit_gate_status`; per clarification Q1=C the handler drops the field before calling the cloud client — see FR-003.) | P1       | Same file, `:53-59`.                                                                      |
| FR-003 | `services/cloud-gate-query-client.ts` `GetGateStatusInput` gains optional `runId`; when present, forwarded as a `runId=<value>` URL query-string parameter (camelCase, per clarification Q2=A) on the cloud call. `ListGatesInput` is **not** widened. The `cockpit_gate_list` handler must NOT forward `runId` to the client — the deployed cloud route (`services/api/src/routes/clusters/cockpit-gates.ts`) carries `.refine((q) => q.runId === undefined \|\| q.generation !== undefined, { message: 'runId requires generation' })`, and list mode has no `generation` by construction, so forwarding `runId` on list produces a 400 RFC-7807, not a silent ignore. Handler drop site MUST carry an inline comment stating the drop is deliberate and naming the cloud refine that requires it. | P1       | `packages/orchestrator/src/services/cloud-gate-query-client.ts`; deployed cloud contract from generacy-cloud#892 merge `192fca7c`. |
| FR-004 | The `cockpit_gate_status` MCP tool handler threads the optional `runId` from parsed input into the cloud query client call. The `cockpit_gate_list` handler parses `runId` (schema accepts it per FR-002) but does NOT pass it to the client (see FR-003). | P1       | `tools/cockpit_gate_status.ts:48-52` and `tools/cockpit_gate_list.ts:51-52`.              |
| FR-005 | With `runId === undefined`, every derived key, id, and outbound URL is byte-identical to the pre-change behaviour. Verification is **dual** (per clarification Q5=C): (a) **snapshot fixture** — check in the exact canonical URL string for the `runId === undefined` case for `getGateStatus` and assert equality (catches percent-encoding drift on `owner/repo#N` where `#` → `%23`, and query-parameter-order drift, both of which FR-005's "byte-identical" language captures literally); (b) **structural assertion** — assert the query-string key set and the absence of `runId` (produces a legible failure message and survives future legitimately-additive parameters without regeneration). | P1       | Backward-compat invariant; guards silent regressions on the legacy caller path.           |
| FR-006 | A regression test pins that `cockpit_gate_open`, `cockpit_gate_status`, AND `cockpit_gate_list` derive from the same inputs — same `gateKey`/`gateId` across all three tools for both the 3-tuple and 4-tuple cases (per clarification Q4=A). Even though `cockpit_gate_list` does not *accept* `runId` on the wire (FR-003), it does *return* `gateId`s derived cloud-side from stored 4-segment `gateKey`s, and the inbox is the surface an operator actually looks at — an open-vs-status-only matrix would let a regression through where open and status agree but the inbox row carries a stale `gateId` (#1053 reappearing at the visible surface). | P1       | Prevents #1053 recurrence at every surface — write, status, inbox.                        |
...
| FR-008 | Structured log line at the tool boundary observes the `runId` source (`explicit` vs `unset`) for `cockpit_gate_status` **only** — `cockpit_gate_list` MUST NOT emit this log (per clarification Q3=C, since list never forwards `runId`, logging `runIdSource` there would assert a provenance for a value that had no effect on the request). Placement is **post-call** with field shape `{ event, runIdSource, mode: 'status', gateType, issueRef, resolvedStatus, gateId }`. The log MUST also be emitted on the **failure path** (transport error, cloud 4xx/5xx) with the error surfaced — a success-only line makes a failing `runId` cycle indistinguishable from one where the tool was never invoked. The `runId` value itself MUST NOT be logged (only the source label). | P2       | Consistency with `cockpit_gate_open.ts:80-97`. `runId` value itself MUST NOT be logged.  |
...
|--------|-----------------------------------------------------------------------------------------------------------|-----------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------|
| SC-001 | No-`runId` byte-compat: outbound URL to cloud for `getGateStatus` with no `runId` matches pre-change. Dual verification per FR-005: (a) snapshot test asserts equality against a checked-in canonical URL string; (b) structural assertion asserts the query-string key set is exactly `{issueRef, gateType, generation}` and `runId` is absent. | 100% (both assertions pass)                           | Snapshot + structural test comparing pre-change and post-change URL construction for a canonical input.                              |
...
| SC-003 | 4-tuple path reaches the cloud: with `runId` supplied to `cockpit_gate_status`, the outbound URL contains `runId=<value>` (camelCase key). `cockpit_gate_list` with `runId` supplied does NOT include the parameter in the outbound URL (handler drops it per FR-003). | 100%                                                | Test with `httpsRequestImpl` seam that captures the request URL for both tools.                                        |
...
| SC-005 | `cockpit_gate_open`, `cockpit_gate_status`, AND `cockpit_gate_list` derive matching `gateKey`/`gateId` for all 3-tuple and 4-tuple cases (per clarification Q4=A). | 100% (matrix test across three tools)                                  | Parameterised test over ≥4 combinations (3-tuple, 4-tuple, distinct `runId` values, empty `generation` boundary), asserting open-vs-status-vs-list-entry identity.       |
...
1. **Cloud Phase A has landed** (generacy-cloud#892, merge `192fca7c`, deployed to staging at 2026-07-29T04:07:07Z; `GENERACY_API_URL` on this cluster is `https://api-staging.generacy.ai`). The cloud endpoint accepts an optional `runId` on `POST /api/clusters/:id/cockpit/gates` and on `GET /api/clusters/:id/cockpit/gates` **in status mode only** (mode is selected by presence of `generation`; the route carries `.refine((q) => q.runId === undefined || q.generation !== undefined, { message: 'runId requires generation' })`). If Phase A has NOT landed, this PR must NOT merge — verified by the on-call reviewer at merge time.
...
7. **List-mode `runId` filtering is deliberately not in scope.** Per clarification Q1=C: the cloud route rejects list requests carrying `runId` with a 400 by construction; adding filter support is a cloud-side change (rejected for Phase B because it would re-invert the A→B→C dependency ordering). The operator use case ("show me this run's gates") is real and should be filed as a follow-up generacy-cloud issue, not fixed here.
8. **Query-string key is camelCase.** The parameter name is `runId` — fixed by the deployed cloud contract (`c.req.query('runId')`), not by convention. Using `run_id` would silently 200 with a wrong-answer `absent` result rather than 400 — the exact failure class #1059 exists to prevent.
