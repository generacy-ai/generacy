...
|----|-------------|----------|-------|
| FR-001 | The remediation budget (`remediationCount`) MUST persist across `address-pr-feedback` re-entries for the same PR by **not clearing the review artifact on re-entry** (or reseeding while preserving the prior `remediationCount`); the review artifact remains the single source of truth for the budget. No separate per-PR budget store is introduced. **[Clarified Q1→A]** | P1 | Root cause: `clearReviewArtifact` at `claude-cli-worker.ts:593` + `prior?.remediationCount ?? 0` at `seed-aware-review-executor.ts:96`. |
...
| FR-002a | The accumulated budget resets to 0 only on (a) an operator resume of the `remediation-limit` gate, or (b) a genuinely new distinct review submission (a new review after the prior human threads were resolved, i.e. the unresolved-thread set changed). Ordinary same-feedback re-entries MUST keep accumulating. **[Clarified Q2→B]** | P1 | The D-2 reset comment at `claude-cli-worker.ts:571-583` already reaches `clearReviewArtifact` on exactly these two occasions. |
| FR-003 | The monitor skip MUST suppress re-enqueue for `failed:*` escalations via a **blanket `failed:*` prefix match** (mirroring the existing `blocked:*` gate, no allow-list). The operator clears it by removing the `failed:*` label (the existing resume convention). **[Clarified Q3→A]** | P1 | `pr-feedback-monitor-service.ts:473,557` (blanket `startsWith('blocked:')` precedent at `:557`, no-allow-list contract at `:449,:488`). |
| FR-004 | Seeded finding `detail` (raw comment bodies) MUST be wrapped with `wrapUntrustedData` **at the ingestion site** before embedding in the remediate charter. Engine-authored review findings are left untouched (no central charter-level wrapping). **[Clarified Q5→A]** | P1 | `seed-aware-review-executor.ts:74-75` → `remediate-charter.ts:54-62`. |
| FR-005 | Validate-evidence finding `detail` (raw validate output tails) MUST be wrapped with `wrapUntrustedData` **at the ingestion site**. **[Clarified Q5→A]** | P1 | `phase-loop.ts:1029-1037`; matches the existing `wrapUntrustedData` pattern at `validate-fix-handler.ts:235`. |
| FR-006 | The `address-pr-feedback` re-entry MUST derive the working branch from the PR's `head.ref` rather than from `createFeature(issueNumber)`. Resolution rule: **exactly one** linked open PR → use its `head.ref`; **zero** → treat as fresh-request (budget 0); **more than one** → park for operator attention. **[Clarified Q4→C]** | P1 | `claude-cli-worker.ts:461-501,482`; authoritative head-ref resolution precedent at `pr-feedback-handler.ts:225`. |
...
- The PR number / `head.ref` is available (or resolvable via `findPRForBranch` / issue-linked PR lookup) at the re-entry point; if the PR does not yet exist, the fresh-request path (budget 0) is correct. Per FR-006 (Q4→C), when more than one linked open PR is found the re-entry parks for operator attention rather than guessing a branch.
