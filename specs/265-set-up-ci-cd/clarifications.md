...
**Answer**: A) Rely on Changesets default. Changesets + pnpm handle `workspace:*` / `workspace:^` replacement and publish ordering automatically. This is the standard approach for pnpm monorepos with Changesets. No need to add explicit topological ordering.
...
**Answer**: A) Use `--if-present`. All current packages have lint/test/build, but `--if-present` is resilient for future packages. A package that defines a script but fails will still fail CI — `--if-present` only skips missing scripts, not failing ones.
...
**Answer**: A) Rely on ephemeral runner. GitHub Actions runners are disposable. The modified package.json files are discarded when the job completes. Adding explicit cleanup is unnecessary noise.
...
**Answer**: A) No path filters — run on everything. Simpler to maintain, avoids stale filter lists, and CI minutes are cheap compared to debugging a missed trigger. A config-only change that breaks the build should be caught.
...
**Answer**: A) Treat as independent. Root eslint explicitly ignores `packages/`, and each package manages its own lint config. This is the correct setup already. No cross-validation needed.
...
**Answer**: A) Root first, then packages. The root `src/` has no `workspace:*` dependencies on any packages — it depends on express, ioredis, uuid. The root build is independent. `pnpm -r run build` then handles topological ordering within packages (workflow-engine -> orchestrator -> generacy).
...
**Answer**: A) Granular access token. Scoped to `@generacy-ai/*` with read-write publish permissions. Smallest blast radius. This is npm's recommended approach for CI.
...
**Answer**: A) Add concurrency controls. Use `cancel-in-progress: true` for CI (no point running stale builds). For publish workflows, use concurrency groups **without** cancellation to prevent partial publishes. This is standard practice.
...
**Answer**: A) Explicit least-privilege permissions. The existing `publish-devcontainer-feature.yml` already uses explicit permissions (`contents: read`, `packages: write`). New workflows should follow the same pattern for consistency and security.
...
**Answer**: A) No linking. Independent versioning is correct here. `updateInternalDependencies: "patch"` already ensures dependents get bumped when their dependencies change. Linking would cause unnecessary version churn.
...
**Answer**: B) Explicit filter. Use `--filter '!generacy-extension'` in publish commands. While Changesets' ignore *should* prevent version bumps, the extension has never been published to npm — if the preview workflow runs `pnpm -r publish` and the extension's 0.1.0 isn't on npm, it would be published unintentionally. An explicit filter is one line and removes all ambiguity.
...
**Answer**: A) Assume it's correct. Verified: `bin/generacy.js` already has `#!/usr/bin/env node` and has executable permissions (`-rwxr-xr-x`). A CI check for this is unnecessary overhead.
...
**Answer**: A) Use `ubuntu-latest`. Node.js and pnpm have no OS-specific library dependencies. Auto-updating keeps security patches current without maintenance burden.
...
**Answer**: A) Add `packageManager` field to root package.json. The root currently has no `packageManager` field. Adding `"packageManager": "pnpm@9.x.x"` (with the exact version from the current lockfile) is the standard approach — it benefits both CI via `pnpm/action-setup` auto-detection and local dev via Corepack.
