...
- **Q1 → A**: When `packages/` is absent, the degraded `preValidateCommand` runs `pnpm install` only (drops the `-r --filter … build` half). Rationale: `pnpm install` on non-pnpm repos is acceptable as a *default*; per-repo overrides / FR-009's scaffolder are the precision instrument.
- **Q2 → A**: The failure evidence block lives *inside* the existing stage comment, rewritten per retry. GitHub's comment-edit history preserves prior failures for the rare archaeology case; the cockpit classifier keeps reading the one surface it already reads.
- **Q3 → D**: The `packages/*` build half fires only when BOTH `pnpm-workspace.yaml` exists AND at least one `packages/*/package.json` exists. Directory-only presence (A) misses the empty-`packages/` transition case; `packages/*/package.json` alone (B) still lets `pnpm -r` die without a workspace file.
- **Q4 → A (with B's marker text)**: Compose bounds as "last 30 lines, then if > 4 KiB truncate from the beginning." Newest bytes at the bottom (terminal-reading order); truncation marker at the top with counts: `… truncated (kept last N lines / M bytes) …`.
- **Q5 → A**: Timeouts and aborts render the full evidence block. Synthesized exit descriptor stands in for the missing exit code (`killed (SIGTERM) after Nms` / `aborted`); an empty stderr surfaces as the literal `(stderr empty)`. Timeouts especially need the stderr tail — the last line often names the stuck step.

## Functional Requirements

| ID     | Requirement                                                                                                                                                                                                                                     | Priority | Notes                                                          |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------------------------------------------------------------- |
| FR-001 | The default `preValidateCommand` MUST NOT hard-fail on a repo without a `packages/` directory. The `pnpm install` half MUST always run; the `pnpm -r --filter './packages/*' build` half MUST run only when BOTH a `pnpm-workspace.yaml` exists at the workspace root AND at least one `packages/*/package.json` exists. Otherwise, only `pnpm install` runs. | P0       | `config.ts:59` + wrapper shell; two fs checks (Q1=A, Q3=D)      |
...
| FR-003 | When a phase fails (including pre-validate install), the GitHub-visible failure surface MUST include: the failing command string, the resolved exit code, and a bounded tail of stderr. The evidence block MUST be appended inside the existing stage comment via `StageCommentManager.updateStageComment` and MUST be rewritten (not appended) on every retry. | P0       | Data already in `PhaseResult.error` (`cli-spawner.ts:247`); Q2=A |
| FR-004 | The stderr tail posted to the issue MUST be bounded by taking the last 30 lines of stderr, then, if that slice exceeds 4 KiB, truncating from the *beginning* (keep newest bytes) and prepending a marker line `… truncated (kept last N lines / M bytes) …`. Marker MUST NOT appear when no truncation occurred. | P0       | Prevents multi-MB comments; Q4=A + B's richer marker             |
| FR-005 | Timeout and abort failures MUST surface their distinct top-level message (`Phase "…" timed out after Nms` / `Phase "…" was aborted`) — not a generic "Error". Timeouts and aborts MUST render the full FR-003 evidence block: distinct top-line message, failing command, a synthesized exit descriptor (`killed (SIGTERM) after Nms` for timeouts, `aborted` for aborts), and the stderr tail — rendering the literal `(stderr empty)` when there is no captured stderr. | P1       | Already in `PhaseResult.error.message`; Q5=A                     |
| FR-006 | The cockpit's `failed:*` classification MUST continue to detect the failure. The evidence block lives inside the same stage comment the cockpit already reads (per FR-003), so the classification pipeline MUST NOT regress.        | P0       | See `cockpit` watch/status; Q2=A resolves surface location       |
