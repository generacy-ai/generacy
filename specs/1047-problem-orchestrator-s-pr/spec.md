...
**Acceptance Criteria**:
- [ ] A fix cycle that leaves ANY gating finding's `**Files:**` list untouched by every commit in the cycle does not advance to Disposition A ("complete"); it advances to disposition C, applies `blocked:body-finding-unaddressed` (FR-007), and posts a marker-keyed PR comment enumerating which specific `### Finding <n>` items (by reviewer, review id, and finding number) remain unaddressed.
- [ ] A fix cycle that touches at least one named file for EVERY gating finding across every gating review advances normally.
- [ ] A superseding review from the same author (newer submission from that login) supplants the earlier one's finding-set; a later review from a different author never supplants another author's finding-set.
- [ ] On resume after the operator removes `blocked:body-finding-unaddressed`, findings previously enumerated in the most-recent `<!-- generacy-cockpit:body-findings-unaddressed -->` marker comment are treated as acknowledged and do not re-gate (per FR-008); the same findings still appear in the fixer prompt via FR-002 for context, and any subsequent NEW review from the same author resurfaces its findings as gating.
...
| FR-003 | Do not mark a fix cycle complete while ANY gating finding has produced no commit touching any file it names. Each `### Finding <n>` sub-block is evaluated as its own gate against its own `**Files:**` list (per-finding, not union): a review is satisfied only when EVERY finding in it has at least one of its named files touched by a commit in the cycle; a single unsatisfied finding blocks the cycle. "Gating review" = per-review-author newest: for each distinct reviewer login with new reviews since the watermark, only that author's newest submission gates; earlier submissions from the same author are superseded by that author's newer one, but never by another author's. | P1 | Disposition-decision change. Extends today's thread-resolution-based logic. Per-finding tracking preserves SC-003 literally — the union reading admits false-completes when a reviewer names files A, B, C and the fixer touches only A. Per-author supersession preserves parallel-review safety: bot and humans post as different logins (`CLUSTER_GITHUB_USERNAME` vs the human), so a human re-review must not silently erase the bot's still-unaddressed body findings. |
...
| FR-007 | When FR-003 blocks the fix cycle from advancing to Disposition A (thread resolves succeeded but at least one gating finding's named files were not touched), apply the new label `blocked:body-finding-unaddressed` and post a top-level PR comment (marker-keyed via `<!-- generacy-cockpit:body-findings-unaddressed -->`) listing which specific `### Finding <n>` items — by review author, review id, and finding number — remain unaddressed. Idempotency: use `listPrCommentBodies` marker-check before posting, matching the existing `maybePostUntrustedNotice` pattern (`pr-feedback-monitor-service.ts:435-480`). Then pause the workflow. This is disposition C — distinct from disposition B (`blocked:stuck-feedback-loop`) so operators can distinguish "fixer looping" from "fixer needs a file outside the diff". | P1 | Free to add: `PrFeedbackMonitorService`'s pre-enqueue skip gate is a bare `l.startsWith('blocked:')` prefix match with no allow-list (`pr-feedback-monitor-service.ts:328-341`), so a new `blocked:` label is honoured with zero monitor-side change. The per-finding operator comment is the shared artifact that also serves FR-008 as the durable acknowledgment record — one artifact, two purposes. |
| FR-008 | On resume (label removed by operator, or a subsequent poll cycle), findings named in the most-recent `<!-- generacy-cockpit:body-findings-unaddressed -->` marker comment on the PR are treated as "acknowledged, do not re-gate" — they still reach the fixer prompt via FR-002 for context, but they do NOT count toward FR-003's per-finding gate on that cycle. Do NOT advance the review watermark. Acknowledged-finding identity keys off `(reviewer login, review id, finding index)` extracted from the marker comment body. A subsequent NEW review from the same author (higher review id) resurfaces the finding as gating; the acknowledgment applies only to the specific reviews it was recorded against. | P1 | Prevents the "operator removes label → same findings immediately re-block" dead end. Rides the existing marker-comment-as-durable-state idiom (no new store required, restart-safe, process-agnostic across the monitor/worker queue boundary). Does NOT advance the watermark because FR-001's "newer than the last cycle" predicate is the single source feeding both FR-002 (prompt) and FR-003 (gate) — advancing would delete the findings from the prompt on the cycle the fixer most needs them. |
...
| SC-006 | Partial-progress false-completes eliminated | 0 | Test: post a body with two `### Finding <n>` blocks (files A and B), stub the fixer to touch only A, assert disposition C is applied and the marker comment lists Finding 2 (file B) as unaddressed. |
| SC-007 | Label-loop after operator resume | 0 | Test: apply disposition C, verify marker comment posted, remove `blocked:body-finding-unaddressed` label, re-run the fixer without any new review, assert FR-003 does NOT re-block on the already-acknowledged findings. |
...
- FR-003 evaluates each `### Finding <n>` sub-block independently against its own `**Files:**` list — the review-level satisfaction check is an AND across findings, not an OR. A finding with no `**Files:**` line (marker absent, or older `/cockpit:auto` poster) contributes zero constraints per FR-005, so a review composed entirely of such findings never gates.
- FR-008's acknowledgment record is a top-level PR comment carrying the `<!-- generacy-cockpit:body-findings-unaddressed -->` marker, listed via `listPrCommentBodies` and posted via `postPrComment`. This choice avoids introducing per-issue in-memory or Redis state, which the monitor deliberately does not maintain across the queue boundary to `PrFeedbackHandler` (constructed fresh per job at `claude-cli-worker.ts:295-299`). The marker comment is the single source of truth for both operator triage (SC-006) and gate suppression (SC-007).
- Finding identity for FR-008 acknowledgment keys off `(reviewer login, review id, 1-based finding index within that review)`. Review ids are stable GitHub identifiers on `pulls/{n}/reviews` payloads; the 1-based index tracks the parsed order of `### Finding <n>` sub-blocks within a single review body. This tuple is stable across parses of the same review and unique across reviews.
