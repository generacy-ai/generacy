...
**Acceptance Criteria**:
- [ ] On a miss against the branch-scoped key, the capture logic reads through to the legacy key format (`phase-start-ref:<owner>:<repo>:<issue>:<phase>`) exactly once and, if a valid SHA is found, reuses it as the phase-start ref.
- [ ] A legacy ref that is consumed is cleared so it cannot be re-read by a later cycle or linger to its TTL.
...
**Acceptance Criteria**:
- [ ] Before a persisted (or legacy-migrated) ref is used as the diff base, its existence in the current checkout is verified (`git cat-file -e <sha>^{commit}` or equivalent).
- [ ] When the ref does not resolve, it is treated as absent: a fresh HEAD is captured and persisted under the branch-scoped key, and the phase proceeds normally.
...
|----|-------------|----------|-------|
| FR-001 | On a miss against the branch-scoped key (`phase-start-ref:<owner>:<repo>:<issue>:<branch>:<phase>`), the capture logic MUST fall back to reading the legacy key (`phase-start-ref:<owner>:<repo>:<issue>:<phase>`) once before capturing a fresh HEAD. | P1 | Migration path for refs written by the pre-#1110 build. Legacy value is subject to the same `isValidCommitSha` shape check. Read-through-on-miss vs. startup drain to be settled at /plan; issue suggests either. |
| FR-002 | A legacy ref that is successfully migrated MUST be cleared (its legacy key deleted) so it is consumed once and cannot be re-read or linger to its 7-day TTL. | P1 | `PhaseTracker.clearRaw` already exists (#1107). |
| FR-003 | Before any persisted ref (branch-scoped or legacy-migrated) is used as the diff base, its existence in the current checkout MUST be verified. A ref that does not resolve MUST be treated as absent. | P1 | Suggested check: `git cat-file -e <sha>^{commit}`. Requires a new "commit exists in checkout" capability on the git/GitHub client — none exists today (only `getCurrentCommitSha`). |
