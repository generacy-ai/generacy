...
**Answer**: Flag Rename Strategy
**Context**: The spec says to "Rename `--template-ref` to something more appropriate (e.g., `--base-ref`) or keep it for backwards compatibility with a deprecation notice." This affects the CLI interface, types, and the `GENERACY_TEMPLATE_REF` env var.

**Question**: Should we rename `--template-ref` to `--base-ref` (clean break), or keep `--template-ref` with a deprecation warning? Additionally, should the `GENERACY_TEMPLATE_REF` environment variable be renamed?

- **A**: Rename to `--base-ref` and `GENERACY_BASE_REF` (clean break, no backwards compat)
- **B**: Keep `--template-ref` / `GENERACY_TEMPLATE_REF` with deprecation warnings, add `--base-ref` / `GENERACY_BASE_REF` as new aliases
- **C**: Keep `--template-ref` / `GENERACY_TEMPLATE_REF` as-is with no rename
...
**Answer**: Default Git Ref for Base Repos
**Context**: The current default ref is `'develop'`. The new `cluster-base`/`cluster-microservices` repos may use a different default branch.

**Question**: What is the default branch for `cluster-base` and `cluster-microservices` — `develop` or `main`?
...
**Answer**: Base Repo File Structure Confirmation
**Context**: `mapArchivePath()` currently strips both the GitHub hash prefix and the variant subdirectory. We need to know the exact file layout in the base repos.

**Question**: In the base repos, are devcontainer files under `.devcontainer/` at the repo root, or directly at the repo root?

- **A**: Under `.devcontainer/` at root (e.g., `.devcontainer/Dockerfile`)
- **B**: Directly at root (e.g., `Dockerfile` at top level)
- **C**: Mixed — some files at root, some in subdirectories
...
**Answer**: Cache Structure and Migration
**Context**: Current cache path is `~/.generacy/template-cache/{ref}/{variant}/`. With per-variant repos, users upgrading could get stale cache hits.

**Question**: Should the cache path structure change, and should old cache be cleaned up?

- **A**: Change cache path to `{repo-name}/{ref}/` — old cache naturally ignored
- **B**: Keep `{ref}/{variant}/` structure — content will be correct since variant → repo
- **C**: Change cache path AND add one-time cleanup of old cache directory
...
**Answer**: Error Message Updates
**Context**: Current error messages reference "cluster-templates". The spec doesn't mention updating them.

**Question**: Should error messages reference the specific repo name per variant, or use a generic term?

- **A**: Use specific repo name (e.g., "cluster-base" or "cluster-microservices")
- **B**: Use generic term (e.g., "base repository")
