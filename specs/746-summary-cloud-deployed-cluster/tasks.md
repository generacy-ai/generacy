...
- [X] T001 [P] [US1] Re-verify in-repo `deriveTunnelName` / `loadOptionsFromEnv` are UUID-keyed by inspecting `packages/control-plane/src/services/vscode-tunnel-manager.ts:60-90`. Confirm post-#744 state matches the spec's assumption. Record the exact line numbers (code may have moved since spec was written) in `specs/746-summary-cloud-deployed-cluster/research.md` under a new "Investigation Log — Layer A code" section. **Done — `deriveTunnelName` lines 64–71, `loadOptionsFromEnv` lines 73–89, both UUID-keyed. Hypothesis #2 (in-repo) eliminated on source side.**

- [ ] T002 [P] [US1] **[manual — requires Droplet SSH]** Probe the regressing Droplet's `GENERACY_CLUSTER_ID` env (hypothesis #1) via SSH per quickstart.md §1.1: `grep -E '^GENERACY_(CLUSTER_ID|PROJECT_ID)=' /opt/generacy/.env` on the live Droplet running cluster `325cdcb9-5b8e-45fc-a1bc-1ec8570d561d` (project `Xr7fxq61PF57U2lOtoKe`). Also run `docker compose -f /opt/generacy/docker-compose.yml exec orchestrator printenv GENERACY_CLUSTER_ID` to capture the runtime value (.env + overrides). Record observed values in research.md under "Investigation Log — Hypothesis #1". If the value is the projectId, mark hypothesis #1 confirmed and root cause = `generacy-cloud cloud-deploy/compose-template.ts` (operational, not template — since template inspection shows UUID is written).

- [ ] T003 [P] [US1] **[manual — requires Droplet SSH]** Verify the installed `@generacy-ai/control-plane` preview tarball actually contains #744's UUID-keyed `deriveTunnelName` (hypothesis #2) via quickstart.md §1.2: `docker compose exec orchestrator node -e "require('@generacy-ai/control-plane/package.json').version"` and `grep -n 'GENERACY_CLUSTER_ID\|GENERACY_PROJECT_ID\|deriveTunnelName' node_modules/@generacy-ai/control-plane/dist/services/vscode-tunnel-manager.js`. Record version + grep output in research.md under "Investigation Log — Hypothesis #2". If the dist file reads `GENERACY_PROJECT_ID`, the running tarball is pre-#744 despite the version string. (Note: T006 already confirmed the published `0.0.0-preview-20260603190235` tarball bytes are post-#744. T003 confirms the Droplet actually has those bytes, not pre-#744 bytes from an earlier `@preview` install.)

- [ ] T004 [P] [US1] **[manual — requires Droplet SSH]** Inspect orchestrator + control-plane logs on the Droplet for the actually-requested tunnel name per quickstart.md §1.3: `docker compose logs orchestrator control-plane | grep -iE 'tunnel.*name|deriveTunnelName|code tunnel --name|vscode-tunnel'`. Record observed `code tunnel --name <X>` invocation and any `cluster.vscode-tunnel` payload (especially `status: 'connected'` events with `tunnelName`) in research.md under "Investigation Log — Cluster request trace". Discrimination: requested name vs actual name vs cloud-doc value pins one of hypotheses #1/#3 or surfaces hypothesis #5 (Microsoft tunnel service auth-bound stale tunnel).

- [X] T005 [US1] FR-001 Layer B read-only inspection: trace the generacy-cloud relay handler that writes `vscodeTunnelName` to `organizations/{orgId}/clusters/{clusterId}`. Per quickstart.md Phase 2 and research.md "Layer B": locate the `cluster.vscode-tunnel` event handler in generacy-cloud (likely under `services/api/.../relay-handlers/`), confirm it reads `data.tunnelName` from the event payload (not computed cloud-side), confirm `preApproveActivationCode` does not seed `vscodeTunnelName` from `projectId`, and confirm write semantics are `merge: true` (not write-once). Record findings in research.md under "Investigation Log — Layer B". Depends on T001-T004 so the cluster-side facts are pinned first. **Done — handler at `message-handler.ts:486–512` reads `data.tunnelName` from event payload; updates only on `status === 'connected'`; `ref.update()` merge semantics; NO cloud-side computation or seed of `vscodeTunnelName` from `projectId` anywhere in non-test code. Cloud-deploy template writes `GENERACY_CLUSTER_ID=${input.clusterId}` (UUID post-#792). Latent multi-cluster concern: `resolveClusterDocRef` looks up by `projectId` with `limit(1)` — separate companion issue candidate, not relevant to single-cluster #746 evidence.**

- [X] T006 [P] [US1] FR-001 Layer C: confirm the `0.0.0-preview-20260603190235` `@generacy-ai/control-plane` tarball was built from a ref containing #744's merge SHA `6f74140` (or descendant). Inspect the relevant `publish-preview` workflow run (manual `workflow_dispatch`) in the `generacy-ai/control-plane` build pipeline or `.github/workflows/publish-preview.*`. Record the actual build ref in research.md under "Investigation Log — Layer C". Can run in parallel with T005. **Done — `publish-preview.yml` run 26906597368 built from headSha `6f74140` (= #744 merge SHA) at 2026-06-03T19:01:40Z, published `0.0.0-preview-20260603190235` at 19:02:35Z. Hypothesis #2 (Layer C) ELIMINATED. (No separate `generacy-ai/control-plane` repo — the package lives in this repo's `packages/control-plane/`.)**

- [/] T007 [US1] FR-002 root-cause synthesis: from T001-T006 results, write a "Diagnosis" section in `specs/746-summary-cloud-deployed-cluster/research.md` that pins the root cause to exactly one of (hypothesis #1 env, hypothesis #2 tarball, hypothesis #3 cache, hypothesis #4 cloud-side computation/seed) and to exactly one Layer (A in-repo, B generacy-cloud, or C release workflow). Cite the specific probe output that proves it. Depends on T001-T006. **Partial — see "Diagnosis (T007) — partial" section of research.md. Layer A and Layer C eliminated; cloud-side computation and seed eliminated (Layer B handler is innocent on the cloud-side compute axis). Remaining candidates require live-Droplet probes T002/T003/T004 to discriminate between: (i) running process pre-#744 despite installed bytes being post-#744, (ii) env override yielding projectId at runtime, (iii) Microsoft tunnel service returning a stale auth-bound tunnel, (iv) post-#744 cluster never reaching `connected` status to overwrite a pre-existing doc value. Full pin gated on T002/T003/T004 operator output.**
