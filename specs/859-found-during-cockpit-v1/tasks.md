...
- [X] T008 [US1] Run `pnpm --filter @generacy-ai/cockpit test` and `pnpm --filter @generacy-ai/generacy test` — all 4 new wrapper tests + 5 new merge tests pass; no pre-existing tests regress. Then walk through `quickstart.md` verification steps (live-repro closure on the christrudelpw/sniplink PR #16 pattern).
