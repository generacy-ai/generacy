...
- [ ] T004 [US1] Add a local reason-format helper inside `packages/orchestrator/src/worker/stage-comment-manager.ts` (private module function, not exported) implementing the normalization defined in data-model.md §Reason format contract and contracts/failure-reason-block.md §Rendering normalization:
...
- [ ] T007 [US1] [US2] Extend `packages/orchestrator/src/worker/__tests__/phase-loop.test.ts` with six new fixtures covering every `buildErrorEvidence` callsite (data-model.md §Classifier Vocabulary, spec.md §Regression tests, plan.md Testing Strategy):
...
- [ ] T008 [P] [US1] Extend `packages/orchestrator/src/worker/__tests__/stage-comment-manager.test.ts` with rendering fixtures per contracts/failure-reason-block.md §Rendering normalization:
...
- [ ] T009 [P] [US1] Add a `.changeset/` markdown entry describing the additive `reason?: string` field on `CommandExitEvidence` plus the `classifier?` parameter on `buildErrorEvidence`. Bump `@generacy-ai/orchestrator` with a patch semver (change is additive; no breaking consumers per plan.md §Rollout).
