...
- [ ] T005 [P] [US2] In `packages/orchestrator/src/services/in-memory-queue-adapter.ts`, add the same `info` log line on the `false`-return path of `enqueueIfAbsent` (around lines 82-105) for adapter parity so the FR-009 contract holds in both runtimes.
...
- [ ] T015 [US1] In `packages/orchestrator/src/services/__tests__/pr-feedback-monitor-service.test.ts`, add SC-005 zero-trusted regression fence: unresolved thread with only untrusted authors → assert no `enqueueIfAbsent` call is made and no queue item is added on any poll. This is the #869 interaction guard that keeps self-clearing dedupe from busy-looping.
