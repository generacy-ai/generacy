...
**Answer**: C) Present year only: `Copyright (c) 2026 The Generacy AI Authors`. Use the year of public release. Year ranges are a maintenance burden and aren't legally necessary. The copyright holder should be "The Generacy AI Authors" (future-proof, covers all contributors). Consistent with latency repo.
...
**Answer**: A) Map all 14 packages. Cover every package in the repo, including `packages/templates/`. The spec's "13" is a typo — CODEOWNERS should be comprehensive and match reality.
...
**Answer**: B) Use team handles (`@generacy-ai/core` and `@generacy-ai/plugins`). Confirm the teams exist in the GitHub org first. Team references are better than individual usernames since membership is maintained in one place. If teams don't exist yet, create them before merging. Consistent with latency repo's approach of using `@generacy-ai/core-team`.
...
**Answer**: A) All plugins share one owner group. With one team at this stage, granular per-plugin rules add noise without benefit. Use a single team owner for all plugins. Add granularity when the team actually grows. Consistent with latency repo's "single default owner" decision.
...
**Answer**: C) Defer the table. State "This project is in early development. All security reports will be evaluated against the latest code on the default branch." Be honest about the pre-1.0 maturity level — users appreciate transparency and it avoids creating support expectations we can't meet yet. Consistent with latency repo.
...
**Answer**: A) Install `gitleaks` binary directly. Download the pre-built Linux binary from GitHub releases — simplest approach with no dependencies. Also persist the gitleaks config (`.gitleaks.toml`) in the repo for future CI integration. Consistent with latency repo.
...
**Answer**: A) Yes, create `.gitleaksignore`. Since we're persisting gitleaks config, adding an allowlist is the natural companion. It prevents the same false positives from flagging in future CI runs and is self-documenting. Consistent with latency repo.
...
**Answer**: C) Commit a summary only. Add a brief summary of findings to the PR description. The audit is a point-in-time artifact tied to the "go public" event — the raw report may contain sensitive path information and doesn't need to be committed. The PR provides the right context and traceability. Consistent with latency repo.
...
**Answer**: A) All root files via wildcard (`/* @owner`). Keep it simple with a catch-all for root files. No need to enumerate individual files — the `*` default owner already covers this. Adding per-file rules for root config adds complexity without benefit at this stage. Consistent with latency repo's "single default owner" approach.
...
**Answer**: A) Yes, map all directories. Comprehensive coverage ensures all PRs get reviewers. Infrastructure code (`docker/`, `scripts/`) especially benefits from code review. Use the same team owner for all. Low cost, high value.
...
**Answer**: A) Audit all and add sensitive ones to `.gitignore`. Before going public, review each tool-config directory for sensitive content (API keys, tokens, personal config). This is a prudent step that goes beyond the spec's minimum scope but directly serves the goal of safe public visibility. Add any directories containing private config to `.gitignore`.
