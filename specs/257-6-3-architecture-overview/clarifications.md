...
**Answer**: D) Replace and redirect. The new adopter-focused doc replaces the existing `docs/docs/architecture/overview.md` as the primary architecture overview in the Docusaurus site. Internal architecture details (data layer choices, cloud deployment topology, etc.) move to a separate "Architecture Internals" page — in the same `docs/docs/architecture/` directory or in `tetrad-development`. This keeps one canonical architecture overview, avoids content drift, and ensures adopters don't stumble into internal details. The Docusaurus path is preferred over a standalone `docs/architecture-overview.md` for navigation, search, and consistent styling.
...
**Answer**: A) Mermaid. Docusaurus already has Mermaid configured, GitHub renders it natively, and existing docs all use Mermaid. No need for ASCII fallbacks.
...
**Answer**: A) Real minimal example. Show a simplified excerpt from `speckit-feature.yaml` (2-3 phases) to ground the explanation in reality, plus a list of built-in action namespaces without full parameter docs. A hypothetical example risks misleading adopters about the actual format.
...
**Answer**: B) Adopter-relevant only. Show labels adopters add (triggers like `feature`, `bugfix`; completion signals like `completed:clarification`, `completed:spec-review`) and labels they'll observe that indicate status or need action (`waiting-for:clarification`, `waiting-for:spec-review`, `agent:error`, `agent:in-progress`). Omit pure system-internal labels (individual `phase:*` transitions, internal routing labels) — they just add noise.
...
**Answer**: A) Minimal setup only. Cover just the essentials to get started — webhook URL, watched repos YAML, API token — and link to the existing config reference (`docs/docs/reference/config/generacy.md`) for everything else. Avoids duplicating the 419-line config reference and prevents content drift.
...
**Answer**: C) Unified pattern. Explain the pattern once (system pauses → posts a comment → adopter reviews → adopter signals completion via label), then list each review type with its specific label and what to look for. Avoids repetition and makes the mental model clear: "there's one pattern, it just applies to different artifacts."
...
**Answer**: B) Mechanism + common scenarios. Cover the mechanism plus 3-4 common error scenarios (timeout, context overflow, test failures, merge conflicts) with specific recovery steps. Gives adopters actionable guidance without over-engineering.
...
**Answer**: B) Brief context. Include a one-liner at the top explaining this doc covers the orchestrated workflow (Level 3+), with a link to the getting-started docs for simpler setups (Level 1-2). The document is written for the Level 3+ audience throughout.
...
**Answer**: C) Hybrid. Conversational tone for narrative walkthrough sections (lifecycle, clarification, review cycles) using second-person ("When you label an issue..."). Reference tone for tables, configuration, and glossary sections to keep them scannable.
...
**Answer**: A) Yes, include. Stage comments are a core part of the adopter experience — they appear on every processed issue and are the primary way adopters see progress. Include a brief explanation in the "Workflow Lifecycle" section (what they look like, that they update automatically as phases complete) — a paragraph or callout within the lifecycle walkthrough, not its own section.
...
**Answer**: A) Self-contained with links. The document must be readable end-to-end without clicking any links, but should add "Learn more" links for readers who want depth. This avoids duplicating the config reference or getting-started guides while still satisfying the requirement that adopters understand the system without additional reading.
...
**Answer**: C) Overview + link. The architecture overview should explain that webhooks are how the orchestrator receives GitHub events and which events are required, but defer step-by-step setup instructions to the getting-started/installation guide.
