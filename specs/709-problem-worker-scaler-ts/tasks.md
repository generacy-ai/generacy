...
- [X] T012 [US1] In `packages/control-plane/src/routes/app-config.ts` (lines ~42–61), replace the `readManifest()` body with `readMergedClusterConfig(await resolveGeneracyDir())`, then validate `merged.appConfig` with the existing `AppConfigSchema`. Preserve ENOENT → `null` behavior when `appConfig` is absent from both files.
- [X] T013 [US1] Update `packages/control-plane/__tests__/routes/app-config.test.ts`: existing fixtures (only `cluster.yaml`) keep passing. Add a regression test where `cluster.local.yaml` exists but lacks `appConfig` — `readManifest()` must still return the canonical `appConfig` unchanged.

## Phase 5: Cross-cutting checks

- [X] T014 [US1] Grep `packages/control-plane/src` and `packages/orchestrator/src` for any remaining direct `cluster.yaml` reads (`readFile.*cluster\.yaml`, `parseYaml.*cluster`, etc.). Migrate any not covered in T010 / T012 onto `readMergedClusterConfig`, or document in this task why they should not be (e.g. CLI-side template parsing in `packages/generacy/src/cli/commands/cluster/context.ts` is host-side, out of scope per plan). **Result**: All in-scope config reads (`worker-scaler.ts`, `relay-bridge.ts:readClusterYaml`, `app-config.ts:readManifest`) go through `readMergedClusterConfig`. Remaining matches are: code comments only; `clusterYamlPath` config field (defaults to `.generacy/cluster.yaml`, used by relay-bridge as basis for `dirname()` then merged-read); `project-dir-resolver.ts` (uses `cluster.yaml` existence as a marker to identify the project dir — not a config read); `routes/files.ts` (general-purpose workspace file route — out of scope per plan).
- [X] T015 [US1] Run full build + test on all affected packages in order: `pnpm --filter @generacy-ai/config build test` → `pnpm --filter @generacy-ai/control-plane build test` → `pnpm --filter @generacy-ai/orchestrator build test`. All green. **Result**: `@generacy-ai/config` build+test green (152/152). `@generacy-ai/control-plane` build+test green (379/379, includes new T013 regression test). `@generacy-ai/orchestrator` build green; test: T011-targeted suite (`tests/unit/services/relay-bridge-metadata.test.ts`) is fully green (6/6 — including new merge override test). 17 other orchestrator tests fail across `relay-bridge.test.ts`, `webhook-setup-service.test.ts`, `poller.test.ts`, `relay-integration.test.ts`, `server-relay-routes.test.ts`, `activate.test.ts` — verified pre-existing on `develop` (same failures reproduce when `relay-bridge.ts` is checked out from `develop`); not introduced by #709.
