...
**Answer**: A — Extend the review executor/charter with an explicit `base..head` diff-window input (new parameter), threaded from the resolution SHAs, so the charter names the exact range. Rationale: the review executor today reads no phase-start-ref and the charter has no window param, so option B's "existing diff-window machinery" does not exist in the review path; reviewing the whole PR unchanged violates FR-002, and an explicit window is the capability the delta-scoped convergence re-reviews need anyway.
...
**Answer**: B — Extend the `re-armed` outcome (and the dispatcher enqueue → `WorkerContext`) to carry the SHAs alongside `startPhase`. Rationale: the re-arm already translates `startPhase` into `rearmItem.metadata` next to `resumeReason`, so adding a `reviewScope:{baseSha,headSha}` field is a minimal extension of an existing, self-scoped channel; the sidecar is cleared immediately after re-arm (so it can't be the source of truth), and a new keyed state entry brings its own staleness lifecycle.
...
**Answer**: B — The scoped-review re-arm is gated by `reviewPhaseEnabled`; when the flag is off, the handler falls back to today's `startPhase: metadata.phase` re-arm. Rationale: with the flag off, phase-loop filters `review` out and throws "Unknown starting phase: review" on a review re-arm, and remediate is still an inert stub — so forcing review on both risks a hard crash and produces a dead-end verdict; gating preserves the safety invariant in both modes and respects the deliberate incremental default-OFF rollout.
...
**Answer**: C — Missing base/head SHA → re-arm `review` anyway with a whole-branch diff fallback; keep `metadata.phase` required for the fail-loud guard. Rationale: the conflict was already resolved and pushed, so fail-louding would strand a genuinely-resolved PR under `blocked:stuck-merge-conflicts` (the terminal-label-strands-the-loop anti-pattern); whole-branch review is a safe superset of the resolution delta and still honors FR-005, and `metadata.phase` is still needed for the Q3 flag-off fallback.
...
**Answer**: A — The engine short-circuits: if the resolution diff window is empty, skip the review executor and proceed directly to `validate` (still satisfies FR-005). Rationale: the charter's "empty diff = blocking finding" rule exists to catch the implement-phase no-op bypass, a rationale that does not apply to a legitimate ours/theirs resolution; an empty base..head window has nothing to review, so running an agent to inspect nothing contradicts the cost-reduction motivation while validate still runs on the post-merge state.
