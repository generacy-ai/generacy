...
- [X] T001 [US-Primary] Add optional `projectId?: string` to the `ActivateOptions` interface in
...
- [X] T002 [US-Primary] In `packages/generacy/src/cli/commands/deploy/index.ts`, resolve
...
- [X] T010 [US-Tertiary] Rewrite `packages/generacy/tests/unit/deploy/activation.test.ts`:
...
- [X] T020 [P] [US-Secondary] Fix the main-entry smoke case in
...
- [X] T030 [P] [US-Primary] FR-005 sibling audit: grep `packages/generacy/tests` and
...
- [X] T031 [P] [US-Primary] Create `.changeset/1190-activation-projectid-purity.md` —
...
- [X] T040 [US-Primary] Verify SC-001 → SC-006 per quickstart.md, from `packages/generacy`:
