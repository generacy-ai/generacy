...
**Answer**: A — shared predicate, trust-aware enqueue. The monitor filters unresolved threads through the same `isTrustedCommentAuthor` the handler uses, so the system never manufactures work the handler will refuse (each of those refusals costs a worker claim, a branch checkout, and a dedupe-key mark — the wedge observed live on PR #14). The predicate should live next to the thread-shaped client from #861 (`getPRReviewThreads`) so both consumers import one function; the monitor's GraphQL query needs the small extension of pulling `author.login` + `authorAssociation` per comment. FR-002/FR-003 loud retention stays in the handler as the fallback for what the monitor can't screen (comment edited/deleted between poll and claim) — defense in depth, not the primary contract. One consequence: with zero-trusted states never enqueued, the FR-004 notice moves to the **monitor**, posted at the transition into "unresolved-but-untrusted" that its existing previous-state tracking already detects.
...
**Answer**: A — hidden HTML marker + grep. This is the established idempotency pattern in this codebase (`<!-- generacy-stage:… -->`, `<!-- cockpit -->`, and #865's marker answer) — "pollutes the conversation with a magic string" isn't an argument we honor anywhere else. B mints a new Redis key with its own TTL/settlement semantics, i.e. the exact machinery class #862 exists to delete; C's 1-per-poll noise trains operators to ignore the notice; D removes the only operator-visible signal. Mechanics: grep PR comments for the marker before posting; one notice per zero-trusted episode; leave old notices in place (audit trail) rather than edit/delete.
...
**Answer**: A — clear on all exceptions. Fail-loud-and-retry beats fail-silent-and-strand, and the busy-loop risk is bounded by the 60s poll cadence — worst case one visibly-failing dispatch per poll, which is *diagnosable*, unlike a TTL strand, which is #861's original pathology. C builds a transient/permanent classification helper into machinery that #862 is scheduled to delete; D preserves the strand for the whole interim window. A is also the only choice forward-compatible with #862, whose in-flight semantics make "cleared on exit" the natural meaning of every terminal path.
...
**Answer**: A — FR-002/FR-003 apply unconditionally; no new error type. Identity-unresolvable is a misconfiguration, so log *that* at `error` level naming the chain links tried (config → `CLUSTER_GITHUB_USERNAME` → `GH_USERNAME` → `gh api user`), but the degraded mode keeps working on association-trusted comments and keeps the zero-trusted path loud (retained label, `warn`, notice). B re-opens the exact wound this spec closes; C escalates a config problem into the worker's error path, which can mark the issue failed and destroys more state than the degradation costs.
...
**Answer**: A — top-level PR comment via `gh pr comment`. It is outside the review-thread scan by construction, so the self-trust loop cannot form and no suppression rule enters the trust predicate (B adds complexity to exactly the function whose subtlety caused this bug). C is actively harmful: a bot resolving threads destroys the operator's own unresolved-conversations signal to solve a problem A avoids for free. Combined with Q1/Q2: the monitor posts the notice at the zero-trusted transition, marker-deduped.
