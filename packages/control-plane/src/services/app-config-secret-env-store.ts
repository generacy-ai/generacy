...
import type { AppConfigFileStore } from './app-config-file-store.js';

const DEFAULT_SECRET_ENV_PATH = '/run/generacy-app-config/secrets.env';
const FALLBACK_SECRET_ENV_PATH = '/tmp/generacy-app-config/secrets.env';
...
/**
 * Atomic read-modify-write store for app-config secret environment variables.
 * Maintains a secrets.env file on tmpfs as a derived view of the encrypted backend.
 * Mirrors AppConfigEnvStore pattern: fallback chain, atomic writes, promise-chain mutex.
 */
export class AppConfigSecretEnvStore {
  ...
  constructor(
    ...
    private readonly fileStore: AppConfigFileStore,
    envPath?: string,
  ) {
    this.envPath = envPath ?? DEFAULT_SECRET_ENV_PATH;
  }
  ...
  async init(): Promise<void> {
    try {
      ...
      const fallbackPath = FALLBACK_SECRET_ENV_PATH;
      try {
  ...
  /**
   * Boot-time full render: walk metadata for secret entries, unseal each,
   * write combined file. Best-effort: skip entries that fail to unseal.
   */
  async renderAll(): Promise<RenderResult> {
    ...
    const meta = await this.fileStore.getMetadata();
    const rendered: string[] = [];
    const failed: string[] = [];
    const entries = new Map<string, string>();

    for (const [name, entry] of Object.entries(meta.env)) {
      if (!entry.secret) continue;
      try {
        const value = await this.backend.fetchSecret(`app-config/env/${name}`);
        entries.set(name, value);
        rendered.push(name);
      } catch (err: unknown) {
        console.warn(
          `[app-config-secret-env] Failed to unseal secret '${name}':`,
  ...
  /** Set a secret env var. Atomic rewrite under advisory lock. */
  ...
  /** Delete a secret env var. Returns true if it existed, false otherwise. */
  ...
  /** List all secret env var names and their values. */
