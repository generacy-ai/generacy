...
export function loadOptionsFromEnv(
  ...
  // Derive the tunnel name from the stable project id, not the per-activation
  // cluster UUID. GENERACY_CLUSTER_ID is regenerated by the cloud's
  // launch-config endpoint on every fresh activation, which would produce a
  // new `code tunnel --name` per session and continuously desync from the
  // cloud's persisted `vscodeTunnelName` (preserved via merge:true on
  // tryRegisterCluster — see generacy-ai/generacy-cloud#563). Using the
  // project id keeps the tunnel name stable across all activations of the
  // same project; the persisted value is always correct, and dashboard URLs
  // resolve to the actually-running tunnel. See #618.
  const id = env["GENERACY_PROJECT_ID"];
  if (!id)
    throw new Error("GENERACY_PROJECT_ID is required for VS Code tunnel");
