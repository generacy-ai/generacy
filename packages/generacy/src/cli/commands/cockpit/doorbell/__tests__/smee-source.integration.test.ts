...
import type { ChecksRollup, SnapshotMap } from '../../watch/snapshot.js';
...
describe('SmeeDoorbellSource integration', () => {
  ...
  // #1113 read-through cache path rows moved to `smee-source.checks-casing.test.ts`
  // so they match the non-integration include glob and gate the speckit `validate`
  // phase — the integration harness excluded them from `pnpm test`.
