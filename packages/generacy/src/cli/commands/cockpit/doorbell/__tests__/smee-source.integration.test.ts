...
describe('SmeeDoorbellSource integration', () => {
  ...
  // casings; the read-mixed rows (2 & 4) go red if `smee-source.ts:375`'s
  // `snapshotKey(ev.repo, 'pr', ev.number)` is ever inlined as
  // `` `${ev.repo}#pr#${ev.number}` `` (payload-canonical casing → cache miss →
  // `checks: undefined`). Casing variants are of `o/r` (not the field-narrative
  // `Painworth/Doc-Intel`) because the mocked epic fixes the watched repo to `o/r`.
