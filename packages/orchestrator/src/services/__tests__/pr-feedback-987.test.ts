...
/**
 * #987 — `setWebhooksConfigured(true, opts?)` contract on PrFeedbackMonitorService.
 */
...
function makeSvc(webhooksConfigured = false, adaptivePolling = true): PrFeedbackMonitorService {
  return new PrFeedbackMonitorService(
    createLogger(),
    clientFactory,
    createQueueManager(),
...
describe('PrFeedbackMonitorService setWebhooksConfigured (#987)', () => {
  ...
  it('2. adaptivePolling option is untouched by the setter', () => {
    ...
    // PR-feedback divisor is 2 → base=60_000 / 2 = 30_000
